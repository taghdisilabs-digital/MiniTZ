...
def installed_identity(root: Path) -> dict:
...
def load_component(root: Path,relative: str,name: str):
    path=root/relative
...
def main(argv=None) -> int:
    ...
    if args.command=="source":
        ...
        return load_component(root,"ops/workstation/biella-resource.py","minitz_resource_adapter").main(args.arguments)
    elif args.command=="serve":
        return load_component(root,"ops/workstation/minitz-os-sandbox/startup.py","minitz_sandbox_startup").main()
    else:
        ...
        qualification=json.loads(path.read_text()) if path.is_file() else {"state":"NOT_OBSERVED"}
