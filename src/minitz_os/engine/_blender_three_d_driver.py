"""Private Blender-side driver for :mod:`minitz.three_d_tool`.
...
_RESULT_MARKER = "MINITZ_3D_RESULT="
_DRIVER_EVIDENCE = "MINITZ_FIXED_DRIVER_V1"
_MAX_OBJECTS = 256
_MAX_NAMES = 256
...
_ANIMATION_MANIFEST_PROPERTY = "minitz_animation_manifest_v1"
...
def _animation_export_manifest(bpy: Any) -> list[dict[str, object]]:
    ...
    for action in sorted(bpy.data.actions, key=lambda item: str(item.name)):
        if "minitz_clip_sha256" not in action:
            continue
        records.append(
            {
                "action_name": str(action.name),
                "baked": bool(action.get("minitz_baked", False)),
                "clip_id": str(action.get("minitz_clip_id", "")),
                "clip_sha256": str(action["minitz_clip_sha256"]),
                "end_frame": float(action.get("minitz_end_frame", 0.0)),
                "expected_fcurve_count": int(
                    action.get("minitz_expected_fcurve_count", 0)
                ),
                "expected_keyframe_count": int(
                    action.get("minitz_expected_keyframe_count", 0)
                ),
                "loop_tolerance": float(
                    action.get("minitz_loop_tolerance", 0.0)
                ),
                "retarget_sha256": action.get("minitz_retarget_sha256"),
                "root_motion_policy": str(
                    action.get("minitz_root_motion_policy", "")
                ),
                "source_skeleton_sha256": action.get(
                    "minitz_source_skeleton_sha256"
                ),
                "start_frame": float(action.get("minitz_start_frame", 0.0)),
                "target_skeleton_sha256": action.get(
                    "minitz_target_skeleton_sha256"
                ),
            }
        )
    if len(records) > 128:
...
def _restore_animation_export_manifest(bpy: Any) -> None:
    ...
    for raw in manifest:
        ...
        seen.add(name)
        action["minitz_baked"] = raw["baked"]
        action["minitz_clip_id"] = clip_id
        action["minitz_clip_sha256"] = clip_sha256
        action["minitz_end_frame"] = raw["end_frame"]
        action["minitz_expected_fcurve_count"] = raw["expected_fcurve_count"]
        action["minitz_expected_keyframe_count"] = raw["expected_keyframe_count"]
        action["minitz_loop_tolerance"] = raw["loop_tolerance"]
        action["minitz_root_motion_policy"] = root_motion_policy
        action["minitz_start_frame"] = raw["start_frame"]
        for key in (
            "retarget_sha256",
            "source_skeleton_sha256",
            "target_skeleton_sha256",
...
def _create(bpy: Any, config: Mapping[str, str | int | float | bool]) -> None:
    ...
    name = str(config.get("name", "MiniTZPyramid"))[:128]
    ...
    material = _material(
        bpy,
        str(config.get("material_name", "MiniTZMaterial"))[:128],
        (
    ...
    if invalid_case == "missing_dependency":
        image = bpy.data.images.new("MissingMiniTZTexture", width=1, height=1)
        image.filepath = "//textures/missing-minitz-texture.png"
        node = material.node_tree.nodes.new("ShaderNodeTexImage")
        node.image = image
    object_value["minitz_source_note"] = str(config.get("source_note", "bounded inert source data"))[:1024]
    bpy.context.view_layer.objects.active = object_value
...
def _environment(
    ...
    root_collection = bpy.data.collections.new("MiniTZEnvironment")
    root_collection["minitz_environment_role"] = "environment"
    ...
    for partition_id in cast(list[str], raw_partitions):
        collection = bpy.data.collections.new(f"MiniTZEnvironmentPartition.{partition_id}")
        collection["minitz_environment_role"] = "partition"
        collection["minitz_partition_id"] = partition_id
        collection["minitz_generator_identity_sha256"] = generator_identity
    ...
    terrain.name = "MiniTZEnvironmentTerrain"
    terrain["minitz_environment_role"] = "terrain"
    terrain["minitz_generator"] = generator
    terrain["minitz_generator_version"] = generator_version
    terrain["minitz_generator_config_json"] = json.dumps(
        generator_config,
        ensure_ascii=False,
        allow_nan=False,
        separators=(",", ":"),
    ...
    terrain["minitz_generator_seed"] = seed
    terrain["minitz_generator_identity_sha256"] = generator_identity
    ...
    for index, item in enumerate(cast(list[Mapping[str, object]], placed)):
        ...
        placed_object.name = f"MiniTZEnvironmentAsset{index}"
        placed_object.location = location
        placed_object.rotation_euler = rotation
        placed_object.scale = scale
        placed_object["minitz_asset_ref"] = cast(str, item["artifact_ref"])
        placed_object["minitz_asset_content_sha256"] = cast(str, item["content_sha256"])
        material_ref = cast(str, item["material_ref"])
        placed_object["minitz_asset_id"] = cast(str, item["asset_id"])
        placed_object["minitz_material_ref"] = material_ref
        placed_object["minitz_variant"] = cast(str, item["variant"])
        placed_object["minitz_parent_ref"] = cast(str | None, item["parent_ref"])
        placed_object["minitz_partition"] = partition_id
        placed_object["minitz_environment_role"] = "placed_asset"
        placed_object["minitz_lod_level"] = 0
        ...
        for lod in range(1, cast(int, layout["lod_levels"])):
            ...
            duplicate["minitz_environment_role"] = "lod"
            duplicate["minitz_lod_level"] = lod
            duplicate["minitz_lod_source_asset_id"] = cast(str, item["asset_id"])
            duplicate["minitz_lod_source_artifact_ref"] = cast(str, item["artifact_ref"])
            duplicate["minitz_lod_source_content_sha256"] = cast(str, item["content_sha256"])
    ...
    if bool(layout["include_collision"]):
        ...
        collision.name = "MiniTZEnvironmentCollision"
        collision.hide_render = True
        collision["minitz_environment_role"] = "collision"
    ...
    if bool(layout["include_navigation"]):
        ...
        navigation.name = "MiniTZEnvironmentNavigation"
        navigation.hide_render = True
        navigation["minitz_environment_role"] = "navigation"
...
def _rig(bpy: Any, skeleton: Mapping[str, object]) -> None:
    ...
    armature.name = "MiniTZCharacterRig"
    armature.data.name = "MiniTZCharacterRigData"
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    for mesh in meshes:
        modifier = mesh.modifiers.new("MiniTZCharacterArmature", "ARMATURE")
...
def _apply_action(bpy: Any, armature: Any, clip: Mapping[str, object], policy: str, *, suffix: str, baked: bool) -> Any:
    action = bpy.data.actions.new(name=f"MiniTZClip:{clip['clip_id']}{suffix}")
    action["minitz_clip_id"] = cast(str, clip["clip_id"])
    action["minitz_clip_sha256"] = _payload_digest(clip)
    action["minitz_root_motion_policy"] = policy
    action["minitz_baked"] = baked
    action["minitz_blend_with_clip_id"] = clip["blend_with_clip_id"]
    action["minitz_blend_factor"] = float(cast(float, clip["blend_factor"]))
    action["minitz_loop_tolerance"] = float(cast(float, clip["loop_tolerance"]))
    action["minitz_source_skeleton_sha256"] = cast(
        str, clip["source_skeleton_sha256"]
    )
    action["minitz_target_skeleton_sha256"] = cast(
        str, clip["source_skeleton_sha256"]
    )
    armature.animation_data_create().action = action
    root_motion = None
    if policy == "extract":
        root_motion = bpy.data.objects.new(f"MiniTZRootMotion:{clip['clip_id']}{suffix}", None)
    ...
    if blend_source is not None:
        source_action = bpy.data.actions.get(f"MiniTZClip:{blend_source}")
        if source_action is None:
            raise ValueError("animation blend source clip is absent from editable source")
        track = armature.animation_data.nla_tracks.new()
        strip = track.strips.new(f"MiniTZBlend:{blend_source}", 0, source_action)
        strip.influence = float(cast(float, clip["blend_factor"]))
    action["minitz_expected_fcurve_count"] = len(
        {cast(str, item["bone_name"]) for item in cast(list[Mapping[str, object]], clip["keyframes"])}
    ) * 9
    action["minitz_expected_keyframe_count"] = len(cast(list[object], clip["keyframes"])) * 9
    frames = [float(cast(float, item["frame"])) for item in cast(list[Mapping[str, object]], clip["keyframes"])]
    action["minitz_start_frame"] = min(frames)
    action["minitz_end_frame"] = max(frames)
    return action
...
def _retarget(bpy: Any, clip: Mapping[str, object], retarget: Mapping[str, object], target: Mapping[str, object], policy: str) -> None:
    ...
    source_action = bpy.data.actions.get(f"MiniTZClip:{clip['clip_id']}")
    if source_action is None:
        raise ValueError("retarget source clip is absent from the editable source")
    target_armature = _new_armature(bpy, target, "MiniTZRetargetRig")
    ...
    action["minitz_clip_sha256"] = _payload_digest(clip)
    action["minitz_retarget_sha256"] = _payload_digest(retarget)
    action["minitz_source_action"] = str(source_action.name)
    action["minitz_target_skeleton_sha256"] = _payload_digest(target)
...
def _animation_facts(bpy: Any) -> dict[str, object]:
    ...
    for action in sorted(bpy.data.actions, key=lambda item: str(item.name)):
        ...
        source_curve_count = int(
            action.get("minitz_expected_fcurve_count", curve_count)
        )
        source_keyframe_count = int(
            action.get("minitz_expected_keyframe_count", keyframe_count)
        )
        if curve_count == 0:
        ...
        actions.append({
            "baked": baked,
            "clip_id": str(action.get("minitz_clip_id", ""))[:128],
            "clip_sha256": str(action["minitz_clip_sha256"]),
            "fcurve_count": curve_count,
            "keyframe_count": keyframe_count,
            "loop_error": loop_error,
            "loop_tolerance": float(action.get("minitz_loop_tolerance", 0.0)),
            "start_frame": float(action.get("minitz_start_frame", 0.0)),
            "end_frame": float(action.get("minitz_end_frame", 0.0)),
            "retarget_sha256": action.get("minitz_retarget_sha256"),
            "root_motion_policy": str(action.get("minitz_root_motion_policy", "")),
            "source_skeleton_sha256": action.get("minitz_source_skeleton_sha256"),
            "source_fcurve_count": source_curve_count,
            "source_keyframe_count": source_keyframe_count,
            "target_skeleton_sha256": action.get("minitz_target_skeleton_sha256"),
        })
...
def _apply_material(
    ...
    material = _material(
        bpy,
        str(config.get("material_name", "MiniTZModifiedMaterial"))[:128],
        (
    ...
    if texture_path is not None:
        ...
        image.name = f"MiniTZBoundTexture:{texture_path}"[:256]
...
def _scene(
    ...
    parent.name = str(config.get("parent_name", "MiniTZSceneRoot"))[:128]
    meshes = _mesh_objects(bpy)
    ...
    if bool(config.get("add_camera", False)):
        ...
        camera.name = str(config.get("camera_name", "MiniTZSceneCamera"))[:128]
    ...
    if bool(config.get("add_light", False)):
        ...
        light.name = str(config.get("light_name", "MiniTZSceneLight"))[:128]
...
def _optimize(bpy: Any, config: Mapping[str, str | int | float | bool]) -> None:
    ...
    for target in _mesh_objects(bpy):
        modifier = target.modifiers.new(name="MiniTZProjectDecimate", type="DECIMATE")
...
def _package_python_tree_sha256(root: Path) -> str:
    ...
    digest = hashlib.sha256(b"minitz-plugin-runtime-tree-v2\0")
    total_size = 0
    for path in paths:
...
def _stage_inputs(
    ...
    stage = tempfile.TemporaryDirectory(
        prefix=f".minitz-3d-{request_sha256[:16]}-",
    )
...
def _inspection(
    ...
    for object_index, object_value in enumerate(all_objects):
        ...
        environment_role = object_value.get("minitz_environment_role")
        if isinstance(environment_role, str):
            ...
            property_names = {
                "artifact_content_sha256": "minitz_asset_content_sha256",
                "artifact_ref": "minitz_asset_ref",
                "asset_id": "minitz_asset_id",
                "generator_config_json": "minitz_generator_config_json",
                "generator_identity_sha256": "minitz_generator_identity_sha256",
                "generator_name": "minitz_generator",
                "generator_seed": "minitz_generator_seed",
                "generator_version": "minitz_generator_version",
                "lod_level": "minitz_lod_level",
                "lod_source_artifact_ref": "minitz_lod_source_artifact_ref",
                "lod_source_asset_id": "minitz_lod_source_asset_id",
                "lod_source_content_sha256": "minitz_lod_source_content_sha256",
                "material_ref": "minitz_material_ref",
                "parent_ref": "minitz_parent_ref",
                "partition_id": "minitz_partition",
                "variant": "minitz_variant",
            }
    ...
    if environment_objects:
        ...
        for collection in sorted(bpy.data.collections, key=lambda item: str(item.name)):
            if collection.get("minitz_environment_role") != "partition":
                continue
            partition_id = collection.get("minitz_partition_id")
            ...
            authoritative_objects = sorted(
                (
                    ...
                    if item.get("minitz_environment_role") == "placed_asset"
                ),
                key=lambda item: str(item.get("minitz_asset_id")),
            )
            partitions.append({
                "artifact_content_sha256s": sorted(
                    str(item.get("minitz_asset_content_sha256"))
                    for item in authoritative_objects
                ),
                "asset_ids": [str(item.get("minitz_asset_id")) for item in authoritative_objects],
...
def _preview(bpy: Any, output: Path, config: Mapping[str, str | int | float | bool]) -> None:
    ...
    camera = bpy.context.active_object
    camera.name = "MiniTZPreviewCamera"
    direction = -camera.location
    camera.rotation_euler = direction.to_track_quat("-Z", "Y").to_euler()
    scene.camera = camera
    ...
    if scene.world is None:
        scene.world = bpy.data.worlds.new("MiniTZPreviewWorld")
...
def _main() -> None:
    ...
    staging_root_name = f".minitz-three-d-stage-{request_sha256}"
