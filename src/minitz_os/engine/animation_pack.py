...
def _capability(name: str) -> Capability:
    return Capability(CapabilityRef(f"animation.{name}", _VERSION), f"Perform bounded animation {name.replace('_', ' ')} work through replaceable adapters.", {"asset": "minitz://contracts/artifact-ref/v1", "project": "minitz://contracts/project-ref/v1"}, {"result": f"minitz://contracts/animation-{name.replace('_', '-')}/v1"}, ("workspace.process.execute", "workspace.filesystem.write", "workspace.artifact.create"), _CREATED_AT)
