...
def _operation_relative(value: object, name: str) -> str:
    ...
    if PurePosixPath(text).parts[0].startswith(".minitz-three-d-"):
...
class _ThreeDService:
    ...
    _RESULT_MARKER = b"MINITZ_3D_RESULT="
    ...
    def _verify_candidate_result(
        ...
        if (
            ...
            != "application/vnd.minitz.3d-report+json"
        ...
        expected_media_type = (
            ...
            if publication.output_observed
            else "application/vnd.minitz.3d-report+json"
        )
        if (
        ...
        try:
            ...
            if result.reality is ThreeDReality.NOT_RUN and not attempted_not_run:
                ...
                if (
                    ...
                    != "application/vnd.minitz.3d-unavailability+json"
            ...
            if durable_output_ref is not None:
                ...
                expected_metadata = {
                    ...
                    "schema_ref": (
                        f"schema://minitz/{request.output_role.replace('.', '-')}/1"
                    ),
    ...
    def _insert_publication(
        ...
        artifact = Artifact(
            ...
            metadata={
                ...
                "schema_ref": f"schema://minitz/{request.output_role.replace('.', '-')}/1",
                "schema_version": "1.0.0",
                "semantic_label": (
    ...
    def _request_content(
        ...
        return self.object_store.put(
            ...
            media_type="application/vnd.minitz.3d-operation+json",
        )

    @staticmethod
    def _staging_output_path(request: ThreeDOperationRequest) -> str:
        return PurePosixPath(
            f".minitz-three-d-stage-{request.request_sha256}",
            request.output_path,
        ).as_posix()

    @staticmethod
    def _staging_root_name(request: ThreeDOperationRequest) -> str:
        return f".minitz-three-d-stage-{request.request_sha256}"
    ...
    def _staged_request_evidence(
        ...
        relative = (
            f"{request.working_directory}/.minitz-three-d-"
    ...
    def _operation_process_request(
        ...
        *,
        ...
        driver_path = Path(
            workspace_path,
            f".minitz-three-d-driver-{request.identity.driver_sha256[:32]}.py",
        )
        request_path = Path(
            workspace_path,
            f".minitz-three-d-{request.request_sha256[:32]}.json",
        )
        if require_files:
    ...
    def _driver_staging_identity(
        ...
        relative = (
            f"{working_directory}/.minitz-three-d-driver-"
    ...
    def _unavailable_result(
        ...
        report_ref = self.object_store.put(
            ...
            media_type="application/vnd.minitz.3d-unavailability+json",
        )
        result = ThreeDOperationResult(
            project_ref=request.project_ref,
    ...
    def blender(
        ...
        *,
        ...
        report_ref = self.object_store.put(
            _json(report).encode(),
            media_type="application/vnd.minitz.3d-report+json",
        )
        if (
            process_result.process_identity is not None
    ...
    def _runtime_process_request(
        ...
        driver_path = Path(
            workspace_path,
            f".minitz-three-d-driver-{identity.driver_sha256[:32]}.py",
        )
        ...
        request_path = Path(
            workspace_path,
            f".minitz-three-d-runtime-{runtime_execution_sha[:32]}.json",
        )
        blender_argv = (
            "--background",
            "--factory-startup",
    ...
    def _runtime_request_path(
        ...
        return (
            f"{working_directory}/.minitz-three-d-runtime-"
    ...
    def _runtime_request_content(
        ...
        return self.object_store.put(
            ...
            media_type="application/vnd.minitz.3d-runtime-probe+json",
        )
    ...
    def _runtime_process_state(
        ...
        try:
            ...
            expected_ref = ContentRef(
                ...
                len(serialized),
                "application/vnd.minitz.process-request+json",
            )
    ...
    def finalize_environment_integration_manifest(
        ...
        assert request.environment_spec is not None
        layout = request.environment_spec
        runtime_ref = "runtime://minitz/" + _digest(cast(Mapping[str, object], report["runtime"]))
        ...
        content = self.object_store.put(raw, media_type="application/vnd.minitz.environment-manifest+json")
        connection = self._connect()
        try:
            row = connection.execute(
        ...
        artifact = self.artifacts.create_artifact(
            ...
            metadata={"media_type": "application/vnd.minitz.environment-manifest+json", "schema_ref": "schema://minitz/environment-manifest/v1", "schema_version": "1.0.0", "semantic_label": "environment-manifest", "semantic_version": "1.0.0"},
        )
        connection = self._connect()
        try:
            connection.execute("BEGIN IMMEDIATE")
    ...
    def _persist_mapping_artifact(
        ...
        raw = _json(payload).encode()
        content = self.object_store.put(raw, media_type="application/vnd.minitz.retarget-mapping+json")
        ...
        artifact = self.artifacts.create_artifact(
            ...
            metadata={
                "media_type": "application/vnd.minitz.retarget-mapping+json",
                "schema_ref": "schema://minitz/retarget-mapping/v1",
                "schema_version": "1.0.0",
    ...
    def persist_retarget_request(
        ...
        content = self.object_store.put(
            raw, media_type="application/vnd.minitz.retarget-request+json"
        )
        ...
        artifact = self.artifacts.create_artifact(
            ...
            metadata={
                "media_type": "application/vnd.minitz.retarget-request+json",
                "schema_ref": "schema://minitz/retarget-request/v1",
                "schema_version": "1.0.0",
