...
class WorkspaceService:
    ...
    def _store_manifest(self, workspace: Workspace, entries: Sequence[WorkspaceFileEntry], *, base: bool) -> ContentRef:
        ...
        return self.object_store.put(_json(payload).encode(), media_type="application/vnd.minitz.workspace-manifest+json")
    ...
    def capture(
        ...
        *,
        ...
        artifact = self.artifacts.create_artifact(
            ...
            metadata={"schema_ref": "schema://minitz/workspace-snapshot/1", "schema_version": "1", "semantic_label": "candidate-workspace-snapshot"},
        )
        connection = self._connect()
        try:
            connection.execute("BEGIN IMMEDIATE")
