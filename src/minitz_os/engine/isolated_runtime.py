...
_REQUEST_MEDIA_TYPE = "application/vnd.minitz.isolated-runtime-request+json"
_RECEIPT_MEDIA_TYPE = "application/vnd.minitz.isolated-runtime-receipt+json"
...
class DockerIsolatedRuntimeAdapter:
        ...
        for operation in _OPERATIONS:
            ...
            capability = self.capabilities.register(
                Capability(
                    ...
                    input_contract={"request": "schema://minitz/isolated-runtime-request/1"},
                    output_contract={"receipt": "schema://minitz/isolated-runtime-receipt/1"},
            ...
            try:
                ...
                implementation = self.implementations.register(
                    ...
                    CapabilityImplementation(
                        ...
                        "adapter://minitz/isolated-runtime",
                        "runtime://oci/container",
                        tool_ref=f"tool://minitz/isolated-runtime/{operation}",
    ...
    def _redaction_environment(
        ...
        for index, secret_ref in enumerate(sorted(expected)):
            key = f"MINITZ_RUNTIME_REDACT_{index}"
            refs[key] = secret_ref
        ...
        *,
        ...
        artifact = self.artifacts.publish_from_run(
            ...
            metadata={
                ...
                "schema_ref": "schema://minitz/isolated-runtime-receipt/1",
    ...
    def _inspect_container(
        ...
        *,
        ...
        expected = {
            "minitz.node_attempt": state.node_attempt_id,
            "minitz.node_fence": str(state.node_fence),
            "minitz.project": state.project_ref.value,
            "minitz.runtime": state.runtime_ref.runtime_id,
            "minitz.runtime_generation": str(state.container_generation),
            "minitz.spec_sha256": state.spec.spec_sha256,
        }
                ...
                artifacts.append(
                    self.artifacts.publish_from_run(
                        ...
                        metadata={
                            ...
                            "schema_ref": "schema://minitz/runtime-output/1",
                            "schema_version": "1.0.0",
                        },
                    )
                )
        ...
        *,
        ...
        listed = self._docker(
            ...
            argv=(
                ...
                "--no-trunc",
                "--filter",
                f"label=minitz.project={access.project_ref.value}",
                "--format",
                "{{.ID}}",
            ),
        ...
        for index, container_id in enumerate(container_ids):
            ...
            runtime_id = labels.get("minitz.runtime")
            generation = labels.get("minitz.runtime_generation")
            ...
            try:
                ...
                if (
                    ...
                    or labels.get("minitz.spec_sha256") != state.spec.spec_sha256
                    or labels.get("minitz.node_attempt") != state.node_attempt_id
                    or labels.get("minitz.node_fence") != str(state.node_fence)
                ):
                    unknown.append(container_id)
                    continue
    ...
    def _container_create_argv(
        ...
        *,
        ...
        argv: list[str] = [
            ...
            container_name,
            "--label",
            f"minitz.project={state.project_ref.value}",
            "--label",
            f"minitz.runtime={state.runtime_ref.runtime_id}",
            "--label",
            f"minitz.runtime_generation={container_generation}",
            "--label",
            f"minitz.node_attempt={state.node_attempt_id}",
            "--label",
            f"minitz.node_fence={state.node_fence}",
            "--label",
            f"minitz.spec_sha256={spec.spec_sha256}",
            "--network",
            "none",
            "--read-only",
            "--cap-drop",
    ...
    def start(
        ...
        *,
        ...
        try:
            ...
            container_name = f"minitz-{state.runtime_ref.runtime_id[3:]}-g{next_generation}"
