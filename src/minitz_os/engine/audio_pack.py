...
def audio_production_pack() -> ProductionPack:
    ...
    capabilities = tuple(Capability(CapabilityRef(f"audio.{name}", "1.0.0"), f"Bounded audio {name}", {"optional": "contract://capability/optional/v1"} if name in AUDIO_OPTIONAL_CAPABILITIES else {}, {"result": f"minitz://contracts/audio-{name}/v1"}, (), "2026-09-01T00:00:00+00:00") for name in names)
