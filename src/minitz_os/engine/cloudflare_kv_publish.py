...
_EVIDENCE_MEDIA_TYPE = "application/vnd.minitz.cloudflare-kv-publish-evidence+json"
_MANIFEST_MEDIA_TYPE = "application/vnd.minitz.cloudflare-kv-package-manifest+json"
...
class CloudflareKvPublishAdapter:
    ...
    def package_key(content_ref: ContentRef) -> str:
        ...
        return f"minitz/packages/{content_ref.digest}"

    @staticmethod
    def manifest_key(manifest_bytes: bytes) -> str:
        if not isinstance(manifest_bytes, bytes):
            raise TypeError("manifest_bytes must be bytes")
        return f"minitz/manifests/{hashlib.sha256(manifest_bytes).hexdigest()}"
        ...
        return _canonical_json(
            {
                ...
                "release_ref": request.release_ref,
                "request_digest": request.request_digest,
                "schema_ref": "schema://minitz/cloudflare-kv-package-manifest/1",
                "source_bindings": [
    ...
    def _finish(
        ...
        *,
        ...
        evidence_payload = {
            ...
            "request_digest": request.request_digest,
            "schema_ref": "schema://minitz/cloudflare-kv-publish-evidence/1",
            "source_bindings": [item.payload() for item in request.manifest.sources],
            "state": state,
            "target_ref": request.target_ref,
            "uploaded": uploaded,
