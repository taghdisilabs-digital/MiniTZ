...
def main()->None:
 ...
 if not prior:
  stored=scene.get("biella_solver_state")
 ...
 scene["biella_solver_state"]=json.dumps(state,sort_keys=True,separators=(",",":"));out=Path(c["output_path"]);bpy.ops.wm.save_as_mainfile(filepath=str(out))
