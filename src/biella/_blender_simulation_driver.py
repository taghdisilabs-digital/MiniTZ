...
def main()->None:
 ...
 scene=bpy.context.scene;scene.frame_start,scene.frame_end=a,b;dt=number(c["timestep"],"timestep")/sub
 for frame in range(a,b+1):
  ...
  for i,p in enumerate(state):
   o=bpy.data.objects.get(f"BiellaParticle_{i:03d}")
   if o is None:bpy.ops.mesh.primitive_ico_sphere_add(subdivisions=1,radius=.03);o=bpy.context.object;o.name=f"BiellaParticle_{i:03d}"
   o.location=p["position"];o.keyframe_insert(data_path="location",frame=frame)
 out=Path(c["output_path"]);bpy.ops.wm.save_as_mainfile(filepath=str(out))
 ...
 Path(str(out)+".json").write_text(json.dumps(m,sort_keys=True),encoding="utf-8");print("BIELLA_SIMULATION="+json.dumps(m,sort_keys=True))
