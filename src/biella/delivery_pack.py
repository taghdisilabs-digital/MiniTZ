...
def delivery_production_pack() -> ProductionPack:
    capabilities = tuple(Capability(CapabilityRef(name, "1.0.0"), f"Bounded delivery {name}", {}, {"result": f"biella://contracts/{name.replace('.', '-')}/v1"}, (), "2026-09-01T00:00:00+00:00") for name in DELIVERY_CAPABILITIES)
