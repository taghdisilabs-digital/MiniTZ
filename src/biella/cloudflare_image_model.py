"""Cloudflare Workers AI image-model adapter with immutable Biella publication."""
...
class CloudflareImageModel:
    ...
    def _artifact(
        ...
        label: str,
    ) -> tuple[ArtifactRef, ContentRef, bytes]:
        ...
        return artifact_ref, content, payload

    def _prompt(
        self,
        access: ProjectAccess,
        specification: ImageSpecification,
    ) -> tuple[str, tuple[ArtifactRef, ContentRef, bytes]]:
    ...
    def _response_image(self, response: CloudflareHttpResponse) -> bytes:
        ...
        if (
            ...
            or response.body.startswith(b"\x89PNG\r\n\x1a\n")
            or response.body.startswith(b"\xff\xd8")
        ):
        ...
        try:
            ...
            raise CloudflareImageModelError(
                "Cloudflare response is neither image bytes nor valid JSON"
            ) from None
        if isinstance(decoded, str):
            ...
            if isinstance(result, dict):
                value = result.get("image", result.get("data", result.get("url")))
            else:
                value = result
        else:
            value = None
        if not isinstance(value, str) or not value:
            raise CloudflareImageModelError("Cloudflare JSON response has no image payload")
        if value.startswith(("http://", "https://")):
            fetched = self._send("GET", value, {}, None)
            if fetched.status < 200 or fetched.status >= 300:
                raise CloudflareImageModelError(
                    f"Cloudflare image URL failed with HTTP {fetched.status}"
                )
            return fetched.body
        encoded = value
        if value.lower().startswith("data:"):
            ...
            if (
                ...
                raise CloudflareImageModelError("Cloudflare image data URI is malformed")
        try:
            return base64.b64decode(encoded, validate=True)
        except ValueError:
            raise CloudflareImageModelError(
                "Cloudflare JSON image payload is not base64"
            ) from None
    ...
    def generate(
        ...
        if not specification.operation.generative:
            raise CloudflareImageModelError("Cloudflare model accepts only generative image operations")
        ...
        boundary, body = _multipart(fields, tuple(reference_files))
        response = self._send(
            "POST",
