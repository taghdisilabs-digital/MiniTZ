...
- Multiple simultaneous customer sandboxes share the same checkpoint. No intermediate customer may resume Biella. The last customer stop verifies no customer remains, source-aligns the canonical repo, verifies the preserved dirty-worktree fingerprint, and restores authoritative production first. Optional Ollama/Qwen restoration failures are recorded separately and never prevent production from resuming.
...
- The steady loop is `RESOLVE -> LOAD BOUNDED CONTEXT -> EXECUTE -> VALIDATE -> LOCAL COMMIT -> ACCEPT -> RECORD PUBLICATION -> ADVANCE`; remote readback is independent. `CONTINUE` stays on the same task/session and must identify real unfinished work.
...
## Audit repair contract — 2026-09-08
- Customer return restores production before optional local AI. Optional start/enablement failures remain resource errors, not production failures.
- Automatic task finalization uses the exact task-owned path/digest set retained in task memory. Directory membership alone never grants ownership; pre-existing unrelated dirty files remain excluded.
- Explicit task-validation JSON and its referenced raw files in the same evidence namespace are retained even when ignored as generated output. Unreferenced scratch is not collected.
- A known source conflict remains RECONCILIATION_REQUIRED across transport failure. Only actual source reconciliation/readback resolves it; unaffected execution continues.
- Any GitHub revision readback mismatch prevents stale Drive publication, without stopping the production task. Verified per-file Drive IDs/digests persist across retries; only missing/changed files are retried, with publication-only backoff and actual provider diagnostics.
- Task-class routing is restored: `ops/local-ai/biella_codex_routing.py` selects the existing class profile against the observed model catalog and call outcomes. No global Reserve preference or force override is installed. Strong task synthesis/closure and bounded-helper limits are unchanged.
- The existing controller observation path refreshes task-owned bytes and the task capsule during execution. Observation failure is nonblocking and cannot rotate, kill, advance or pause a task.
- Live task status is task-ID-bound. A predecessor result cannot mark the current task complete. Validation display requires an actual task-matching JSON record plus exact digest, never keywords in a search command.
- These corrections add no permanent reviewer, scheduler, approval hook, silence timer or progress gate. External-player, target-platform and commercial proof still require real evidence.

## Drive five-task transfer routine — 2026-09-08
- Mahdi-directed cadence: routine Drive publication runs after five additional distinct canonical tasks reach COMPLETE or COMPLETE_ALREADY. Attempts, shell events, retries, reconciliation commits, and reconnects never count. Existing completed tasks form the migration baseline; a final partial group flushes only when the canonical program is exhausted.
- Local task state, ledger, accepted output commits, automatic advancement and GitHub publication remain per-task. Drive accumulation is BATCHING, not a stalled task or a false remote-success claim.
- The existing publication cursor retains the completed-ID baseline and one pending Drive batch. New GitHub revisions can proceed during a package upload. Failed batches remain pending and retain exact verified-file receipts; no task is replayed.
- First transfer is an exact committed Git source/evidence snapshot; subsequent batches contain committed changes plus current control records and explicit deletion paths relative to the last verified package. Dirty work, untracked scratch, model weights, caches and unrelated VPS directories are not exported. Referenced final artifacts outside Git retain their task-defined publication route.
- Packages are gzip-compressed Git tar streams split into parts of at most 3,800,000,000 bytes (3.8 GB, decimal). Small/final parts are not padded. Files larger than one part are reconstructed by ordered concatenation; the manifest records part order, sizes, SHA-256, source commit/tree, task IDs and delta base/deletions.
...
`EXECUTOR_OWNS_ROUTINE_BLOCKER_RESOLUTION`: routine implementation/environment/tool/resource blockers are executor work. Use existing authorized Resources, reversible setup, capability routing, bounded cleanup/provisioning, and exact handoffs directly; do not ask Mahdi to perform operations the executor can execute. Only a genuine product decision, unavailable credential/entitlement, or authority for a new paid/external Resource may be escalated. Such escalation never blocks independent GAME_FIRST work.
