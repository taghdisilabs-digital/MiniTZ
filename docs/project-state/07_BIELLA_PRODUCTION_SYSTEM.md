...
- A bounded fallback turn uses a fresh executor session and a retrieval-first packet referencing current task memory, the compact projection, the task guide, and exact source/evidence. It does not resume or replace the preserved persistent task session; bounded executor session IDs are never written into persistent task-session identity. It never broad-rereads accumulated conversation history.
