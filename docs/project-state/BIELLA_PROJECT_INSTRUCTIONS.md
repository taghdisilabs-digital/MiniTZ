...
ISOLATED_PROJECT_CELL: Biella owns task execution; project-specific state remains inside the Project cell. chatgpt_remote is a replaceable Resource receiving only credential-free scoped inputs. Customer credentials/source/Git/Cloudflare/brand/rules/history remain separate. Only verified sanitized project-neutral engineering lessons become candidates. First customer obtains a child-free checkpoint; protected services sleep; last customer restores exact task/session/work. Clear acknowledged pause requests before restart.
...
D_NEXT_100_TASKS.json maps D05-01–D23-05: source digests, dependencies, objectives, deliverables, validation, execution roots. Load one entry; old FUTURE_BLOCKED/repository/controller envelopes stay inactive. D_TASK_LEDGER.json is derived, not a queue.
...
DURABLE_FAILURE_LEDGER: retain failures.jsonl and publication receipts. LOSSLESS_MEMORY_COMPACTION creates content-addressed unique records as rebuildable derivatives; preserve raw authority/evidence/failures. VERIFIED_ACTION_MEMORY reuses accepted evidence only. Cache failure must never stall, pause, reset or advance execution. Never invent a destination.
