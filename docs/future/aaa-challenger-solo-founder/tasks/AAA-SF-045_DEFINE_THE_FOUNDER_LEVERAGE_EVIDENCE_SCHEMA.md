# AAA-SF-045 — Define the founder-leverage evidence schema

Status: `FUTURE_BLOCKED`
Phase: `P6` — Solo-founder leverage measurement
Project: Biella Engine
Repository: `patrickminitz-web/biella-engine`
Predecessor: `AAA-SF-044` durably closed
Next task: `AAA-SF-046`
...
Define the minimal durable measurements needed to quantify AI-native execution leverage without creating a second analytics authority or leaking project-private content.
...
Versioned evidence schema for tasks, runs, accepted outcomes, founder interventions, elapsed time, model/provider/resource use, cost where available, and recovery events.

## Validation contract

Schema maps to current Engine Task/Run/Resource/Event concepts or existing semantic equivalents; it does not create project-specific permanent Engine types.

## Required evidence

Schema/source commit and mapping to current Engine interfaces.

## New execution-map evidence inputs

The schema must be able to represent the current task-packet/context-receipt evidence without creating a parallel analytics authority: context bytes/tokens where available, context expansions and reasons, repeated reads/rediscovery, retries, reuse, routing outcome, validation failures, task duration, accepted result, and recovery events. Preserve current Engine type boundaries; add no project-specific permanent kernel types merely for investor reporting.
