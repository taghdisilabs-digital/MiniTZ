# AAA-SF-063 — Capture real demand signals from the proof surface

Status: `FUTURE_BLOCKED`
Phase: `P8` — Demand and commercial proof
Project: Biella Games
Repository: `patrickminitz-web/biella-games`
Predecessor: `AAA-SF-062` durably closed
Next task: `AAA-SF-064`
...
Collect only actual opt-in signals available on the accepted distribution/public surface, such as test requests, followers, wishlists if a store is accepted, or qualified inbound interest.
...
Demand-signal dataset with source, timestamp, and definition for each metric.

## Validation contract

Every metric comes from a real platform/source record; platform-specific metrics are not invented when that platform is not active.

## Required evidence

Export/readback from demand source and dataset digest.
