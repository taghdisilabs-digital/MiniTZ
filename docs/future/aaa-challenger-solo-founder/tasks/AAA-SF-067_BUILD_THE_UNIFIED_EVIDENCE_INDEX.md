...
Create one index linking Games product proof, external player proof, founder leverage metrics, Engine replication, demand, and contracted usage to exact source evidence.
...
Machine- and human-readable evidence index.

## Validation contract

Every claim has an evidence state and exact pointer; duplicate copies do not become new authority.

## Required evidence

Index digest, referenced Git/Drive/artifact identities.

## New evidence-index requirements

Index the six investor proof lanes explicitly: live objective-to-outcome, external replication, cross-model continuity, durable recovery, contracted/paid usage, and repeatable deployment. Include context/task-packet identities where they materially support reproducibility. Historical normalized candidates and old state snapshots must be labeled by evidence class and never presented as current implementation proof.
