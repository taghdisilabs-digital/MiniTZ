...
Assemble a minimal chain from exact source to clean build to clean-machine play so an outside technical reviewer can reproduce the claim.
...
Reproducibility evidence bundle and index.

## Validation contract

Every link source -> build -> package -> install -> play -> persistence -> diagnostics is backed by exact identities; missing proof remains explicitly unresolved.

## Required evidence

Evidence index, digests, Git commit/tree, Drive readback.
