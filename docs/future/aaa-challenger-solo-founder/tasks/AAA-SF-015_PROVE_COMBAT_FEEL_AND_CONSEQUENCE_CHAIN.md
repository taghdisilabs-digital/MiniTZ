# AAA-SF-015 — Prove combat feel and consequence chain

Status: `FUTURE_BLOCKED`
Phase: `P2` — AAA vertical-slice proof
Project: Biella Games
Repository: `patrickminitz-web/biella-games`
Predecessor: `AAA-SF-014` durably closed
Next task: `AAA-SF-016`
...
Exercise real input -> weapon/action -> hit resolution -> damage -> reaction -> state change during the canonical slice.
...
Combat-quality evidence and bounded repair changes where current behavior fails acceptance.

## Validation contract

Combat actions are responsive and legible, consequences are real simulation state, and no visual-only effect is counted as gameplay completion.

## Required evidence

Raw combat capture, telemetry/log events, source/build identity.
