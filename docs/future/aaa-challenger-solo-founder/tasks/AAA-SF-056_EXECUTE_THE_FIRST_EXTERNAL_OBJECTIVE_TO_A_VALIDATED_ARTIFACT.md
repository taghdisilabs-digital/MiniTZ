# AAA-SF-056 — Execute the first external objective to a validated artifact

Status: `FUTURE_BLOCKED`
Phase: `P7` — Biella Engine external replication
Project: Biella Engine
Repository: `patrickminitz-web/biella-engine`
Predecessor: `AAA-SF-055` durably closed
Next task: `AAA-SF-057`
...
Run the pilot objective through real resources and produce the contracted output with task-derived validation.
...
Validated pilot artifact and execution trace.

## Validation contract

Artifact matches the pilot output contract and is linked to exact Task/Run/Graph/source/resource identities; agent claim alone is insufficient.

## Required evidence

Artifact identity/digest, run/events, validation evidence.

## New source/reuse constraint

Capture the external objective as a compact current task packet and execute it through current Biella. Evidence must show objective -> real execution -> validation -> durable artifact/state, with exact context/source identities and no founder-created hidden substitute result.
