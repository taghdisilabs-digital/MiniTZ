# AAA-SF-057 — Persist and reconstruct the external pilot state

Status: `FUTURE_BLOCKED`
Phase: `P7` — Biella Engine external replication
Project: Biella Engine
Repository: `patrickminitz-web/biella-engine`
Predecessor: `AAA-SF-056` durably closed
Next task: `AAA-SF-058`
...
End the active process/session and prove the external project can be reconstructed from durable state.
...
External pilot continuation record.

## Validation contract

Project/task/run state reappears correctly without relying on chat context, RAM, temporary cache, or founder memory.

## Required evidence

Shutdown/restart evidence, persisted object identities, reconstructed state readback.

## New source/reuse constraint

Reconstruction proof must use durable Project/Task/Run/artifact/evidence state and the recorded context receipt; a surviving chat, process, terminal, model cache, or temporary workspace is not reconstruction evidence.
