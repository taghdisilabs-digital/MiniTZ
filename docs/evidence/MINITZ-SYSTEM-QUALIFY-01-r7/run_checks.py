"""Bounded revision-7 qualification. Never boots the disk image."""
import datetime
import hashlib
import json
...
def run(name, label=None):
    ...
    if name == "structural":
        ...
        script = {"clean-target": "MINITZ-SYSTEM-QUALIFY-01-r7/clean_target.py",
            "payload": "MINITZ-SYSTEM-QUALIFY-01-r6/image_payload.py",
            "preinstall": "MINITZ-SYSTEM-QUALIFY-01-r7/preinstall.py"}[name]
    ...
    record = {"task_ref": "task://minitz/MINITZ-SYSTEM-QUALIFY-01/7", "command": command,
