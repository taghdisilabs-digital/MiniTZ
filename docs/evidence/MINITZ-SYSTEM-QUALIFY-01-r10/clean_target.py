...
with tempfile.TemporaryDirectory(prefix="minitz-qualification-") as temporary:
    ...
    continuity = provision_first_boot(system,
        task_state={"current_task_ref": "task://minitz/MINITZ-SYSTEM-QUALIFY-01/10"},
    ...
    def cli(*args, with_task_program=False):
        command_env = dict(env)
        if with_task_program:
            # This command reports canonical task state, so supply its explicit
            # read-only input. Other installation/recovery commands stay isolated.
            program_path = Path("/state/task-program/TASK_PROGRAM.json")
            command_env["MINITZ_TASK_PROGRAM_PATH"] = str(program_path)
            result["task_program_input_sha256"] = hashlib.sha256(program_path.read_bytes()).hexdigest()
        started = time.monotonic()
        completed = subprocess.run([sys.executable, "-B", "-S", "-m", "minitz_os", *args],
            cwd=work, env=command_env, capture_output=True, text=True, timeout=60)
    ...
    result["capabilities_observation"] = cli("capabilities", with_task_program=True)
