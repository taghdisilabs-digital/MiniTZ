...
try:
    from .minitz_control_gateway import APPROVED_CAPABILITIES
except ImportError:
    from minitz_control_gateway import APPROVED_CAPABILITIES
...
class WorkstationState:
    def __init__(self, *, repo: Path,
        ...
        self.repo = Path(repo)
        self.games_project = self.repo / "projects" / "minitz-games"
    ...
    def _git_files(self, lane: str, limit: int = 80) -> list[str]:
        ...
        if lane == "Website":
            ...
            paths = [path for path in paths if path.startswith("projects/minitz-games/")]
        elif lane == "Engine":
            paths = [
                path for path in paths
                if not path.startswith("website/")
                and not path.startswith("projects/minitz-games/")
    ...
    def overview(self, lane: str) -> dict[str, object]:
        ...
        return {
            "lane": lane,
            "goal": f"Operate the current {lane} lane from its verified source and live MiniTZ runtime.",
    ...
    def _production_status(self) -> dict[str, object]:
        rc, output = self.commands(["/usr/local/bin/minitz-codex", "production", "status"], None, 15)
        if rc != 0:
    ...
    def resources(self) -> list[dict[str, object]]:
        rc, output = self.commands(["/usr/local/bin/minitz-resource", "status"], None, 20)
        if rc != 0:
            return []
        try:
            payload = json.loads(output)
    ...
    def capabilities(self, lane: str) -> dict[str, object]:
        descriptions = {
            "workstation.status": "Read compact MiniTZ workstation status.",
            "workstation.doctor": "Run the bounded MiniTZ workstation diagnostic suite.",
    ...
    def _provider_items(self) -> list[dict[str, str]]:
        rc, output = self.commands(["/usr/local/bin/minitz-workstation", "providers"], None, 30)
        if rc != 0:
            return []
    ...
    def hardware(self, lane: str) -> dict[str, object]:
        ...
        qwen = next((item for item in ps.get("models", []) if item.get("name") == "qwen3-coder-next:minitz" or item.get("model") == "qwen3-coder-next:minitz"), {})
        ...
        usage = [
            ...
            {"name": "Qwen VRAM", "value": f"{vram or 0} bytes", "detail": "qwen3-coder-next:minitz"},
            {"name": "Context mode", "value": os.environ.get("MINITZ_CONTEXT_MODE", "progressive"), "detail": "context loading policy"},
            {"name": "Context file budget", "value": os.environ.get("MINITZ_CONTEXT_MAX_FILES", "8"), "detail": "soft files per expansion"},
            {"name": "Context byte budget", "value": os.environ.get("MINITZ_CONTEXT_MAX_BYTES", "65536"), "detail": "soft bytes per expansion"},
            {"name": "Context log tail", "value": os.environ.get("MINITZ_CONTEXT_LOG_TAIL_LINES", "120"), "detail": "maximum routine log tail lines"},
            {"name": "Context search results", "value": os.environ.get("MINITZ_CONTEXT_SEARCH_RESULTS", "20"), "detail": "maximum routine targeted results"},
        ]
        return {
            "lane": lane,
    ...
    def workers(self, lane: str) -> dict[str, object]:
        rc, state = self.commands(["systemctl", "is-active", "minitz-ollama.service"], None, 10)
        ollama_status = "READY" if rc == 0 and state.strip() == "active" else "DEGRADED"
        ps = self.http_json("/api/ps")
        qwen_ok = any(item.get("name") == "qwen3-coder-next:minitz" or item.get("model") == "qwen3-coder-next:minitz" for item in ps.get("models", []))
        ...
        rc_tunnel, tunnel = self.commands(["systemctl", "is-active", "minitz-control-tunnel.service"], None, 10)
        items = [
            {"name": "Ollama", "status": ollama_status, "detail": "minitz-ollama.service"},
            {"name": "Qwen", "status": "READY" if qwen_ok else "DEGRADED", "detail": "qwen3-coder-next:minitz"},
            ...
            {"name": "Cloudflare tunnel", "status": "CONNECTED" if rc_tunnel == 0 and tunnel.strip() == "active" else "OFFLINE", "detail": "minitz.taghdisilabs.digital ingress"},
        ]
        return {"lane": lane, "items": items}
    ...
    def milestones(self, lane: str) -> dict[str, object]:
        ...
        if lane == "Website":
            ...
            ready = self._ref_has_path(lane, "ops/workstation/minitz")
