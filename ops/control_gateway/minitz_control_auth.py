...
try:
    from .minitz_control_gateway import password_record
except ImportError:
    from minitz_control_gateway import password_record

DEFAULT_AUTH_FILE = Path("/root/attached-storage/minitz-os-sandbox/state/credentials/control/auth.json")
...
def configure(path: Path) -> None:
    ...
    os.chown(tmp, 0, 0)
    tmp.replace(path)
    print("MiniTZ control authentication configured.")
...
def main(argv: list[str]) -> int:
    path = Path(os.environ.get("MINITZ_CONTROL_AUTH_FILE", DEFAULT_AUTH_FILE))
    ...
    if command == "status":
        status(path)
        return 0
    print("usage: minitz-control-auth [configure|status]", file=sys.stderr)
    return 2
