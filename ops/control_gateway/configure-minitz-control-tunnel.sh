...
readonly HOSTNAME="minitz.taghdisilabs.digital"
readonly TUNNEL_NAME="minitz-control"
readonly ORIGIN="http://127.0.0.1:8787"
readonly CONFIG_DIR="/etc/cloudflared"
readonly CONFIG_FILE="$CONFIG_DIR/minitz-control.yml"
readonly CONTROL_INSTALL_DIR="${MINITZ_CONTROL_INSTALL_DIR:-/usr/local/lib/minitz-control}"
readonly SERVICE_SOURCE="$CONTROL_INSTALL_DIR/minitz-control-tunnel.service"

[[ "$EUID" -eq 0 ]] || { echo 'run tunnel configurator as root' >&2; exit 1; }
[[ -f "$RUNTIME_ENV" && ! -L "$RUNTIME_ENV" ]] || { echo 'MiniTZ runtime credentials missing' >&2; exit 1; }
[[ "$(stat -c '%u:%a' "$RUNTIME_ENV")" == "0:600" ]] || { echo 'MiniTZ runtime credentials must be root-owned 0600' >&2; exit 1; }
set -a
...
work_dir="$(mktemp -d /tmp/minitz-control-cloudflare.XXXXXX)"
...
cf_get "/zones?name=taghdisilabs.digital" "$zone_json"
zone_id="$(python3 - "$zone_json" <<'PY'
import json,sys
d=json.load(open(sys.argv[1])); r=d.get('result',[])
if len(r)!=1: raise SystemExit('expected exactly one taghdisilabs.digital zone')
print(r[0]['id'])
PY
)"
...
if len(r)>1: raise SystemExit('multiple tunnels share the MiniTZ control name')
...
PY
  ...
  credentials_file="$CONFIG_DIR/${tunnel_id}.json"
  [[ -f "$credentials_file" ]] || { echo 'existing MiniTZ control tunnel has no local credentials; refusing replacement' >&2; exit 1; }
fi

config_tmp="$(mktemp "$CONFIG_DIR/minitz-control.yml.XXXXXX")"
cat > "$config_tmp" <<EOF
tunnel: $tunnel_id
...
if [[ "$dns_state" == CONFLICT ]]; then
  echo 'conflicting DNS record exists for minitz.taghdisilabs.digital; refusing overwrite' >&2
  exit 1
...
install -o root -g root -m 644 "$SERVICE_SOURCE" /etc/systemd/system/minitz-control-tunnel.service
systemctl daemon-reload
systemctl enable --now minitz-control-tunnel.service >/dev/null

for attempt in $(seq 1 30); do
  if curl -fsS --max-time 10 "https://${HOSTNAME}/v1/control/session" > "$work_dir/readback.json" 2>/dev/null; then
    echo "MiniTZ control tunnel connected: https://${HOSTNAME}/control/"
    exit 0
  fi
  sleep 2
done
