...
class LiveProjection:
    ...
    def sanitize_event(cls, event: dict[str, object]) -> dict[str, object] | None:
        ...
        if event_type == "agent.message":
            text = str(event.get("text") or "MiniTZ updated production state").strip()
            try:
    ...
    def _efficiency_status(self, task_id: str) -> dict[str, object]:
        ...
        active = (
            projection.get("schema") == "minitz.compacted_task_projection/v1"
    ...
    def start(
        ...
        if self._thread and self._thread.is_alive():
            return
        self._stop.clear()
        self._thread = threading.Thread(target=self._refresh_loop, name="minitz-live-projection", daemon=True)
        self._thread.start()
