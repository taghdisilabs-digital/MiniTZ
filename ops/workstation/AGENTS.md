# MiniTZ Unified Main Coder Policy

`biella-codex` remains the compatibility/operator entrypoint for the single MiniTZ production controller. Codex and Antigravity/AGR are peer main-coder execution backends beneath that one controller; neither backend is progression authority.
Codex native sessions remain under its configured Codex home and AGR native conversations remain under its Antigravity installation, but both consume the same canonical Task Program, Project authority, task-memory capsule, compacted current-task projection, evidence/failure journals, content-addressed MiniTZ caches, and AGENTS policy. Provider-native sessions are acceleration only and never become canonical memory.
Changing main coder, model, provider, terminal, native session, or worker does not reset verified project progress. If one main coder reaches observed usage exhaustion, continue the same task through the other ACTIVE coder from shared MiniTZ continuity without replay. If both are ACTIVE, use the second coder concurrently only for a distinct non-overlapping read-only or isolated support unit; canonical source mutation remains single-writer.
...
The MiniTZ main-coder controller chooses local Qwen, external APIs, GPU, Unreal, GitHub, Drive, Cloudflare, Modal, Saturn, deterministic tools, and other Resources only when materially useful.


## Main coder pool — Codex + AGR

`MAIN_CODER_POOL`: Codex and Antigravity/AGR are the two owner-authorized peer main coders. Whatever MiniTZ execution policy applies to Codex at the task/memory/cache/evidence/validation/resource/recovery layer applies equally to AGR unless a rule is explicitly provider-native. Do not maintain separate provider-authored project memory, task ledgers, acceptance logic, or divergent instruction files. The runner injects the same engine/project AGENTS paths and shared MiniTZ task state into either backend.

`MAIN_CODER_FOUR_STATE`: each main coder exposes exactly `OFFLINE`, `ACTIVE`, `OUT_OF_CREDIT`, or `NEEDS_MODIFICATION` as its top-level service state. `ACTIVE` means actually usable and must be considered where useful. `OUT_OF_CREDIT` requires observed usage/credit exhaustion. `NEEDS_MODIFICATION` means the backend is present/online but MiniTZ cannot successfully use it, or failed to route it when applicable. Diagnostic detail may explain the state but may not invent a fifth status.

`MAIN_CODER_HANDOFF`: task identity, current bytes, task-memory capsule, compacted projection, evidence, failure history, Project AGENTS, tool/resource state, and MiniTZ content-addressed caches are canonical shared continuity. Codex thread IDs and AGR conversation IDs are stored separately per task. A native session ID from one backend must never be passed to the other. On provider change, start/resume the destination backend from shared MiniTZ continuity and redo only invalidated work.

`MAIN_CODER_PARALLELISM`: when both Codex and AGR are `ACTIVE`, use both when two useful independent subproblems exist. The primary owns the canonical write path. The peer runs read-only or in an explicitly isolated write boundary and returns an authority-`NONE` content-addressed assist result. The writer never waits for optional peer output; late/stale/failed peer work is ignored or reused later when compatible. Never run duplicate equivalent work merely to keep both coders busy.

`MAIN_CODER_CACHE_UNIFICATION`: provider-native token/session caches remain implementation details of their providers and cannot be merged across vendors. MiniTZ-level task capsules, projections, source/material/result caches, verified actions, failure records, and content-addressed peer assists are shared by Codex and AGR. A backend handoff must maximize reuse of these shared caches without pretending one provider owns the other's private cache.
...
Project Memory, Engine Memory, Run Memory, Historical Evidence and Cache remain distinct. Codex and AGR provider-native homes/sessions are only execution substrates; neither owns or splits MiniTZ semantic memory/cache scopes.
...
Provider discipline: use one external provider by default; fan out only for a unique capability, required fallback, material independent validation, or the owner-authorized Codex+AGR main-coder parallel path. Prefer deterministic/local execution when sufficient; the MiniTZ controller chooses the tool/resource based on the task rather than a model-specific workflow.
...
The Progressive Auto Feeder inside `biella-codex production` may choose Codex or AGR as the main-coder backend and then choose an eligible backend model from observed task class and availability. Backend/model selection does not create a second controller, writer, memory, cache, validation, or progression authority.
...
AGR/Antigravity is the peer main-coder backend reached through the installed `agy` CLI. Use its locally discovered model catalog; do not hard-code availability from historical observations. Select quality by task class: lightweight Flash for simple bounded work when sufficient, strong Sonnet/Pro-class models for normal creation/integration, and the strongest available Opus/Pro-class route for hard/deep-memory/high-risk creation. Creation never uses low reasoning merely to conserve usage.
AGR usage/credit state is learned only from real task/peer execution results, never quota-probe calls. A successful `agy models` listing proves catalog visibility, not successful inference. Provider-native AGR conversation IDs are stored only under the AGR slot for that task and must never be passed to Codex.
...
Production runtime telemetry under `/mnt/biella-extra/biella-runtime/codex-production/` may record only liveness, project/task identity, PIDs, active main coder/model/reasoning, per-coder four-state status and bounded diagnostics, provider-native task session references, peer-assist process/result references, cooldowns, attempt sequence, last result, heartbeat, and update time. Runtime telemetry never overrides canonical Task Program/project task completion and never becomes semantic memory authority.
...
Use the strongest appropriate ACTIVE main-coder route for section planning/audit and deep-memory frontier resolution. Astra Ultra remains the preferred Codex route when available; AGR uses its quality-first task-class model routing. Section planning must stay inside the current accepted section, avoid overlapping tasks, and never invent product scope from stale TODOs or historical evidence.
...
- `SINGLE_MINITZ_PROGRESSION_AUTHORITY`: one MiniTZ canonical task execution/write lease owns source mutation, validation admission, completion, and task advancement. Codex and AGR are interchangeable main-coder backends, never separate authorities. Both may be ACTIVE concurrently, but only one is the canonical writer for an overlapping source scope; the other may execute independent read-only or isolated support work. Codex internal subagent fan-out remains disabled for production.
...
Target 98% cached input where the actual workload permits; this is an optimization objective, never a completion claim or execution blocker. Measure provider-reported cached input from actual Codex or AGR usage when available, distinguish per-request from cumulative ratios, and never pad prompts or make dummy calls to improve the percentage.
...
Use the configured local Qwen Resource for bounded source/log preprocessing and assistance when correct and useful; retain source digests and reuse matching cached assistance. The MiniTZ canonical writer remains the single authority for source edits, synthesis, validation admission and advancement; Codex/AGR backend identity never changes that authority.
...
`ASTRA_UNAVAILABLE_IS_NONBLOCKING`: route the same preserved task through AGR and all other eligible resources before treating Astra absence as a remaining resource condition. Only after all eligible main-coder/resource paths have proven all executable implementation and non-Astra validation done, with no known unsatisfied executable criterion and only Astra-specific final confirmation required, return `CONTINUE` whose summary begins exactly `REQUIRES_OTHER_RESOURCE: ASTRA_FINAL_CONFIRMATION_UNAVAILABLE`. Do not falsely mark the task complete. The existing controller-native resource-deferral path preserves the pending task/session/evidence and advances immediately to dependency-runnable canonical work.
...
`PUBLIC_SPINE_SLEEP_EXCLUSION`: owner sleep/pause applies only to private MiniTZ production/model services. Caddy, `biella-public-site.service`, `biella-control-gateway.service`, and `biella-website-live-deploy.path` are outside the sleep boundary and must remain ACTIVE. Use `/usr/local/bin/minitz-owner-sleep` for owner sleep; ad-hoc `systemctl disable --now` lists that include public services are forbidden.
...
## COMMANDER_ASSIST_FABRIC — 30 non-authoritative channels

MiniTZ exposes exactly 30 logical Commander lanes, `CMD-01 through CMD-30`, as a shared read-only assist fabric around the one canonical writer. Commander lanes have authority NONE. They may inspect only the bounded current Task/Run memory projection supplied to them and may return findings/evidence references/candidate actions; they never edit task files, mutate task/order/status, validate completion, commit, push, publish, or own a provider/session as progression authority.

The canonical writer never waits for Commander lanes. Zero Commander results must still permit normal task progress. Lane/process/provider failure is nonblocking and is preserved only as assist evidence. Every lane is content-addressed by current task identity, task-state digest, compacted projection digest, lane role, and requested provider; identical work is reused rather than reissued. Durable leases exist only to suppress duplicates and recover dead/expired assist processes; leases carry no execution or progression authority.

Local Qwen is reserved for the canonical writer by default and is excluded from Commander dispatch so assist traffic cannot starve production. Commander lanes use eligible external `llm.fast` Resources and may occupy up to 30 logical channels when distinct useful work and provider capacity exist; per-provider concurrency is bounded independently. Each provider begins with one useful canary lane and ramps only after a validated result; each lane lease binds one exact provider with hidden resource failover disabled, and provider failure enters bounded backoff rather than flooding parallel calls. The existing up-to-two TaskBoosters remain a separate focused assist mechanism and are not counted among the 30 Commander lanes.

Commander/provider/lane top-level status uses only `OFFLINE`, `ACTIVE`, `OUT_OF_CREDIT`, or `NEEDS_MODIFICATION`. Activity such as running, useful, rejected, unassigned, or terminated is diagnostic only. Shared MiniTZ memory, compacted context, evidence identity, caches, failure history, AGENTS policy, and Task/Run identity survive Commander process/provider changes; provider-native token/session caches are never misrepresented as shared.
