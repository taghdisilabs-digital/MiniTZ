...
# shellcheck source=minitz-lib.sh
...
curl_config_header() {
  ...
  cfg="$(mktemp "$MINITZ_RUNTIME_ROOT/tmp/curl.XXXXXX")"
  body="$(mktemp "$MINITZ_RUNTIME_ROOT/tmp/provider.XXXXXX")"
  chmod 600 "$cfg" "$body"
  {
    printf 'silent\nshow-error\nconnect-timeout = 4\nmax-time = 20\n'
    printf 'output = "%s"\nwrite-out = "%%{http_code}"\n' "$body"
...
[[ -n "${EXA_API_KEY:-}" ]] && curl_config_post_json_header Exa 'https://api.exa.ai/search' '{\"query\":\"MiniTZ provider health\",\"numResults\":1}' 0 "x-api-key: $EXA_API_KEY" || not_configured Exa
