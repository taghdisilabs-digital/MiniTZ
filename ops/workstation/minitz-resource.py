...
REGISTRY_PATH = Path(os.environ.get("MINITZ_PROVIDER_REGISTRY", str(_CANONICAL_REGISTRY if _CANONICAL_REGISTRY.is_file() else Path(__file__).with_name("provider-registry.json"))))
...
def load_registry(path: Path = REGISTRY_PATH) -> dict[str, Any]:
    ...
    if data.get("schema") != "minitz.provider_registry/v1" or not isinstance(data.get("providers"), dict):
...
def _resource_loop_state_path(env: Mapping[str, str]) -> Path:
    ...
    return Path(raw) if raw else Path("/root/attached-storage/minitz-os-sandbox/state/resource-loop/rotation.json")
...
def _http_json(method: str, url: str, headers: Mapping[str, str], body: Mapping[str, Any] | None,
    ...
    req_headers = {"Accept": "application/json", "User-Agent": "MiniTZ-Resource/1"}
...
def _model_for(provider_id: str, provider: Mapping[str, Any], env: Mapping[str, str], requested: str | None) -> str:
    if requested:
        return requested
    override = env.get(f"MINITZ_{provider_id.upper()}_MODEL", "").strip()
    if override:
        return override
    ...
    if isinstance(default, str) and default:
        return default
    raise ResourceError(
        f"provider {provider_id} requires --model or MINITZ_{provider_id.upper()}_MODEL",
...
def _run_fast_llm_once(
    ...
    *,
    ...
    if provider_id == "ollama-qwen":
        base = str(env.get("MINITZ_OLLAMA_URL", "")).strip().rstrip("/")
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="minitz resource")
