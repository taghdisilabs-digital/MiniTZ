...
def _http_json(method: str, url: str, headers: Mapping[str, str], body: Mapping[str, Any] | None,
    ...
    try:
        ...
        raise ResourceError(f"provider HTTP {exc.code}") from None
    except (urllib.error.URLError, TimeoutError, OSError, json.JSONDecodeError) as exc:
        raise ResourceError(f"provider request failed: {type(exc).__name__}") from None
    if not isinstance(parsed, dict):
        raise ResourceError("provider returned non-object JSON")
    return parsed
...
def _model_for(provider_id: str, provider: Mapping[str, Any], env: Mapping[str, str], requested: str | None) -> str:
    ...
    if isinstance(default, str) and default:
        return default
    raise ResourceError(f"provider {provider_id} requires --model or BIELLA_{provider_id.upper()}_MODEL")
...
def run_fast_llm(registry: Mapping[str, Any], prompt: str, *, env: Mapping[str, str] | None = None,
    ...
    try:
        ...
        raise ResourceError("provider response missing assistant content") from None
