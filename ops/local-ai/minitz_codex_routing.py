...
_LOCAL_OSS_MODEL = "qwen3-coder-next:minitz"
...
def select_route(task_class: str, catalog: Mapping[str, set[str]], cooldowns: Mapping[str, str], now: datetime, *, excluded_models: set[str] | None = None) -> Route:
    ...
    excluded.update(
        item.strip()
        for item in re.split(r"[,\s]+", os.environ.get("MINITZ_CODEX_EXCLUDE_MODELS", ""))
        if item.strip()
    )
    forced_model = os.environ.get("MINITZ_CODEX_FORCE_MODEL", "").strip()
    forced_reasoning = os.environ.get("MINITZ_CODEX_FORCE_REASONING", "").strip()
    if forced_model:
    ...
    preferred = os.environ.get("MINITZ_CODEX_PREFER_MODEL", "").strip()
    if preferred:
    ...
    for route in recovery_routes:
        ...
        if route.model in catalog and route.reasoning in catalog[route.model]:
            return route
    local_model = os.environ.get("MINITZ_CODEX_LOCAL_MODEL", _LOCAL_OSS_MODEL)
...
def _discover_local_ollama_models() -> set[str]:
    ollama_bin = os.environ.get("MINITZ_OLLAMA_BIN", "/usr/local/bin/ollama")
    try:
...
def discover_catalog(*, cache_path: Path | None = None, timeout_seconds: float = 2.0) -> dict[str, set[str]]:
    codex_bin = os.environ.get("MINITZ_CODEX_BIN", "/usr/bin/codex")
    result: dict[str, set[str]] = {}
    try:
        proc = subprocess.run(
...
def local_model_catalog_path() -> Path:
    configured = os.environ.get("MINITZ_CODEX_LOCAL_MODEL_CATALOG")
    return Path(configured) if configured else Path(__file__).with_name("minitz-qwen-codex-model-catalog.json")
