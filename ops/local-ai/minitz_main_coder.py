...
def copilot_candidate_available() -> bool:
    path = Path(os.environ.get("MINITZ_COPILOT_BIN", "/usr/local/bin/copilot"))
...
def build_copilot_peer_command(
    ...
    copilot_bin = os.environ.get("MINITZ_COPILOT_BIN", "/usr/local/bin/copilot")
    command = [
...
def _protected_runtime_values(keys: Iterable[str], base_env: Mapping[str, str]) -> dict[str, str]:
    ...
    if not missing:
        return result
    source_path = Path(
        str(base_env.get("MINITZ_CREDENTIAL_SOURCE") or base_env.get("MINITZ_CREDENTIAL_ENV") or "/root/attached-storage/minitz-os-sandbox/state/credentials/runtime.env")
    )
    try:
...
def copilot_peer_env(profile: str, base_env: Mapping[str, str] | None = None) -> dict[str, str]:
    ...
    if profile == "native":
        ...
        if model.lower() == "auto":
            raise ValueError("MiniTZ native peer requires an explicit model")
    ...
    if profile == "local-qwen":
        env.update({
            ...
            "COPILOT_MODEL": str(source.get("MINITZ_LOCAL_MODEL") or "qwen3-coder-next:minitz"),
        })
        return env
...
def classify_copilot_observation(returncode: int, output: str) -> str:
    ...
    if int(returncode) == 127 or _OFFLINE_RE.search(text):
        return "OFFLINE"
    if int(returncode) == 0:
        return "ACTIVE"
