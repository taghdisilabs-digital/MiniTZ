...
def proven_execution_style() -> dict[str, Any]:
    return {
        "schema": "biella.proven_execution_style/v1",
...
def proven_execution_style_prompt() -> str:
    return (
        ...
        "- MODEL_SILENCE_IS_NOT_FAILURE: never rotate or kill a healthy model turn because it is quiet.\n"
...
_FORBIDDEN_ACTIVE_LITERALS = {
    "timer_executor_rotation": ("BIELLA_EXECUTOR_STALL_ROTATION", "task.executor_stall_recovery"),
    "no_progress_wait_state": ("WAITING_FOR_STRONG_MODEL", "bounded_no_progress"),
    "optional_local_ai_startup_gate": (
        "Requires=biella-ollama.service",
        "ExecStartPre=/usr/local/lib/biella-workstation/biella-qwen-ready.sh",
    ),
    "installer_enablement_toggle": ("systemctl disable biella-codex-production.service",),
}
...
def _audit_cli(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(prog="biella-execution-style")
