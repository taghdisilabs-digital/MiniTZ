...
from minitz_production_state import ProductionState, TaskRecord, active_task_path
...
def compile_task_packet(repo_root: Path, production: ProductionState, task: TaskRecord) -> str:
    if production.priority_policy == "MINITZ_TASK_PROGRAM":
        ...
        return (
            ...
            "AI peer assists such as Copilot, Copilot+Cloudflare, local-Qwen-backed Copilot, plus direct local Qwen resource use, are optional authority NONE support for the canonical writer; peer failure must not block task progress. "
    ...
    return (
        "MiniTZ canonical production task. Execute one task boundary only.\n"
        ...
        f"TASK: {task.id} [{task.task_class}] {task.title}\n"
        "ACTIVE_CONTRACT: docs/project-state/04_MINITZ_ACTIVE_TASK.md\n"
        "--- ACTIVE CONTRACT ---\n"
        f"{contract}\n"
        "--- END ACTIVE CONTRACT ---\n"
        ...
        "`minitz resource route <capability>` and configured specialized Resources when they materially reduce time/tokens or improve quality. "
        ...
        "For every final deliverable, preserve the exact canonical local file and publish it to the configured canonical destination defined by current Project/task authority; verify exact remote identity and bytes/digest when supported. Never invent a destination. The controller independently retries canonical GitHub source publication from exact committed bytes. Transport loss does not require another implementation turn. Actual deployment or remote-delivery task acceptance still requires its real external evidence. Record genuine delivery failures in `/root/attached-storage/minitz-os-sandbox/state/production/failures.jsonl` without fabricating success. "
        "Commit this task's implementation/evidence locally before returning COMPLETE; the Auto Feeder owns canonical GitHub source publication and state transition. "
...
def compile_resume_packet(task: TaskRecord, capsule_path: Path) -> str:
    packet = (
        ...
        "Do not stop for confirmation or routine permission when this task is already authorized. Final deliverables must remain at their canonical local path and be published/read back at any configured canonical destination; never invent a destination. Diagnose prior failures from `/root/attached-storage/minitz-os-sandbox/state/production/failures.jsonl` when relevant. Commit coherent task-owned implementation/evidence; the controller can finalize an omitted task-local commit without task replay. Return CONTINUE only for actual unmet task work, not a continuity publication retry. "
...
def compile_section_packet(production: ProductionState, section, *, audit: bool) -> str:
    ...
    return (
        "MiniTZ production section planning boundary.\n"
        ...
        "Use minitz resource routing when specialized Resources reduce model work. Do not create approval or owner-decision gate tasks for already-authorized work; express executable missing work directly and provision routine task needs as part of execution. "
