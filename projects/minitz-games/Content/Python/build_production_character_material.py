...
-D03UpdateCharacterFill changes only the derived graph's authored fill scalar.
"""
import json
import os
...
UPDATE_FILL = '-D03UpdateCharacterFill' in unreal.SystemLibrary.get_command_line()
assert not (VERIFY and UPDATE_FILL), 'Choose authoring or read-only verification'
...
if not ASSETS.does_asset_exist(path):
    ...
    fill.set_editor_property('slider_max', 0.12)
...
if UPDATE_FILL:
    emission = LIB.get_material_property_input_node(material, unreal.MaterialProperty.MP_EMISSIVE_COLOR)
    multiply = LIB.get_inputs_for_material_expression(material, emission)[1]
    fill = LIB.get_inputs_for_material_expression(material, multiply)[1]
    assert str(fill.get_editor_property('parameter_name')) == 'CharacterReadabilityFill'
    fill.set_editor_property('default_value', FILL)
    fill.set_editor_property('slider_max', 0.12)
    assert not LIB.recompile_material(material)
    assert ASSETS.save_loaded_asset(material, only_if_is_dirty=False)
...
Path(os.environ['BIELLA_D03_ANIMATION_REPORT']).write_text(json.dumps(dict(
    result='PASS', mode='readback' if VERIFY else 'update_fill' if UPDATE_FILL else 'author', material=path,
