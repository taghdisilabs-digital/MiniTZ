#!/usr/bin/env python3
"""Author the panel graph, then verify its saved bytes in a fresh commandlet."""
import argparse
import json
...
def main():
    ...
    report=dict(task_id='D17-01',result='FAIL',runs=[],sources=[file_identity(PROJECT/p) for p in (
        'Content/Python/author_service_panel.py','SourceAssets/Materials/ServicePanel.hlsl')])
    try:
        for mode in ('author','readback'):
            previous=assets()
            command=['runuser','-u','unreal','--','env',f'BIELLA_D17_PANEL_REPORT={out}/{mode}.json',
                str(DEFAULT_EDITOR.with_name('UnrealEditor-Cmd')),str(PROJECT/'BiellaGames.uproject'),
                '-run=pythonscript',f'-script={PROJECT}/Content/Python/author_service_panel.py',
                '-EnablePlugins=PythonScriptPlugin','-unattended','-nullrhi','-nosound','-nop4','-stdout','-FullStdOutLogOutput']
            if mode=='readback': command.append('-D17VerifyServicePanel')
            ...
            assert 'D17_PANEL_ASSET COMPLETE' in log
        ...
        assert set(changed)=={'Content/Environment/ServiceBay/M_ServicePanel.uasset'},sorted(changed)
        ...
        with LEDGER.open('a') as f: f.write(json.dumps(dict(task_id='D17-01',time=now(),type='panel_asset_authoring',
