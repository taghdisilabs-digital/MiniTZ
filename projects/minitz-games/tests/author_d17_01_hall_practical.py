#!/usr/bin/env python3
"""Author the hall practical and verify saved packages in a separate native process."""
import argparse
import json
import pwd
...
def main():
    ...
    report = dict(task_id='D17-01', result='FAIL', inputs_before=before, runs=[],
                  author=file_identity(PROJECT/'Content/Python/author_hall_practical.py'),
                  spec=file_identity(PROJECT/'SourceAssets/Environment/hall-practical.json'))
    try:
        for mode in ('author', 'readback'):
            previous = identities()
            cmd = ['runuser', '-u', 'unreal', '--', 'env', f'BIELLA_D17_HALL_PRACTICAL_REPORT={out}/{mode}.json',
                str(DEFAULT_EDITOR.with_name('UnrealEditor-Cmd')), str(PROJECT/'BiellaGames.uproject'),
                '-run=pythonscript', f'-script={PROJECT}/Content/Python/author_hall_practical.py',
                '-EnablePlugins=PythonScriptPlugin', '-unattended', '-nullrhi', '-nosound', '-nop4',
                '-stdout', '-FullStdOutLogOutput']
            if mode == 'readback': cmd.append('-D17VerifyHallPractical')
            ...
            assert not runtime_has_task_error(log) and 'D17_HALL_PRACTICAL COMPLETE' in log
        ...
        assert authored['light'] == readback['light'] and authored['support'] == readback['support']
        assert readback['support']['soffit_z_cm'] <= readback['support']['housing_top_z_cm'] <= 652
        assert authored['spec_sha256'] == readback['spec_sha256'] == report['spec']['sha256']
        allowed = {'Content/Maps/BiellaOpenWorldMap.umap', 'Content/Environment/ServiceHall/SM_HallPractical.uasset'}
        ...
        with LEDGER.open('a') as f:
            f.write(json.dumps(dict(task_id='D17-01',time=now(),type='hall_practical_authoring',status='FAILED',
