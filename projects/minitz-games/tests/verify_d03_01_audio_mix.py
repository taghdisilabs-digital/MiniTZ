...
def measure(folder):
    ...
    # A loud critical cue can dominate direct combat cross-correlation. Remove
    # its fitted contribution before locating combat, then jointly fit both
    # raw references. No expected ducking gain is used to choose either lag.
    critical_scale = float(np.sum(critical * arrays['mixed']) / np.sum(critical * critical))
    combat, lag_c = aligned(arrays['combat'], arrays['mixed'] - critical_scale * critical)
...
def qualify_mix(mix, expect_unducked=False):
    """Reject poor source fits before interpreting the measured gain."""
