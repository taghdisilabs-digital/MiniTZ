#!/usr/bin/env python3
"""Author hall cladding and verify saved packages in a separate native process."""
import argparse
import json
import pwd
...
def main():
    ...
    report = dict(task_id='D17-01', result='FAIL', inputs_before=before, runs=[],
                  author=file_identity(PROJECT/'Content/Python/author_service_hall.py'),
                  spec=file_identity(PROJECT/'SourceAssets/Environment/service-hall.json'))
    try:
        for mode in ('author', 'readback'):
            previous = identities()
            cmd = ['runuser', '-u', 'unreal', '--', 'env', f'BIELLA_D17_HALL_REPORT={out}/{mode}.json',
                str(DEFAULT_EDITOR.with_name('UnrealEditor-Cmd')), str(PROJECT/'BiellaGames.uproject'),
                '-run=pythonscript', f'-script={PROJECT}/Content/Python/author_service_hall.py',
                '-EnablePlugins=PythonScriptPlugin', '-unattended', '-nullrhi', '-nosound', '-nop4',
                '-stdout', '-FullStdOutLogOutput']
            if mode == 'readback': cmd.append('-D17VerifyHall')
            ...
            assert not runtime_has_task_error(log) and 'D17_HALL COMPLETE' in log
        ...
        allowed = {'Content/Maps/BiellaOpenWorldMap.umap', 'Content/Environment/ServiceHall/SM_ServiceHallCladding.uasset'}
        ...
        with LEDGER.open('a') as f:
            f.write(json.dumps(dict(task_id='D17-01',time=now(),type='hall_authoring',status='FAILED',
