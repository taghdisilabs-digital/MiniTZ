#!/usr/bin/env python3
"""Inspect real editor buffers using existing environment/streaming fixtures.
...
def main():
    ...
    parser.add_argument('--fixture', choices=('Environment', 'WorldStreaming'), default='Environment')
    ...
    cmd = ['runuser', '-u', account.pw_name, '--', 'env', 'SDL_AUDIODRIVER=dummy',
           ...
           f'-AbsLog={out / "runtime.engine.log"}',
           f'-Biella{"Environment" if args.fixture == "Environment" else "Streaming"}Output={out}',
           f'-ExecCmds={controls},Automation RunTests BiellaGames.D02.{args.fixture}; SoftQuit']
    write_json(out / 'validation.json', report)
    try:
        report['runtime'] = monitor(cmd, out, 250 if args.fixture == 'Environment' else 720)
        ...
        assert re.search(r'Result=\{Success\}.*Name=\{' + args.fixture + r'\}', log)
        ...
        report['verification'] = (verify if args.fixture == 'Environment' else verify_streaming)(out)
