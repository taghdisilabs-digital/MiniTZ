...
def finalize_log(path, timeout=45):
    """Wait for inherited writers, including Unreal's detached trace daemon.

    Process-group exit alone does not close a descriptor inherited by a daemon
    in a new session. Never terminate shared services; wait only on this inode.
    """
    target = path.stat()
    ...
    while True:
        writers = []
        for directory in Path("/proc").glob("[0-9]*"):
            for descriptor in (directory / "fd").glob("*"):
                try:
                    info = descriptor.stat()
                    if (info.st_dev, info.st_ino) != (target.st_dev, target.st_ino):
                        continue
                    fdinfo = (directory / "fdinfo" / descriptor.name).read_text()
                    flags = int(next(line.split()[1] for line in fdinfo.splitlines() if line.startswith("flags:")), 8)
                    if flags & os.O_ACCMODE == os.O_RDONLY:
                        continue
                    identity = (int(directory.name), descriptor.name)
                    observed.add(identity)
                    writers.append({"pid": identity[0], "fd": identity[1]})
                except (OSError, ValueError, StopIteration):
                    continue
        elapsed = time.monotonic() - start
        if not writers or elapsed >= timeout:
            return {"closed": not writers, "elapsed_seconds": elapsed, "timeout_seconds": timeout,
                    "observed_writers": [{"pid": pid, "fd": fd} for pid, fd in sorted(observed)],
                    "remaining_writers": writers}
...
def main():
    ...
    try:
        ...
        require(runtime["log_finalization"]["closed"], "Inherited stdout writers exceeded log-finalization bound")
