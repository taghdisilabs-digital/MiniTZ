...
def main():
    ...
    report = dict(task_id='D03-01', result='FAIL', revision=source_revision(), scope='critical-cue combat ducking',
                  capture_status='GENERATED_DRAFT', fixture='Native gameplay/audio mixer using SDL dummy output; population and pose frozen, fixed simulation delta 1/60, audio follows wall time; not speaker or shipping qualification',
                  identities_before=inputs(), protected_before=[file_identity(x) for x in protected])
    try:
        command = ['runuser','-u','unreal','--','env','SDL_AUDIODRIVER=dummy','xvfb-run','-a','-s','-screen 0 1280x720x24',
