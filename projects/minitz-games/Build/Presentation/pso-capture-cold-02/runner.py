...
def main():
    ...
    try:
        ...
        if args.seed:
            ...
            destination = state/'user/Saved/BiellaGames_VULKAN_SM6.upipelinecache'
        ...
        command += ['-psocache', '-logpso']
        if args.file_cache_with_precaching:
            command.append('-DPCVars=r.Vulkan.EnablePSOFileCacheWhenPrecachingActive=1')
        ...
        assert re.search(r'Result=\{Success\}.*Name=\{Surfaces\}', log), 'Native surfaces failed'
        ...
        assert 'Forcing PSO cache from command line' in log and 'Forcing logging of PSOs from command line' in log
        if args.file_cache_with_precaching:
        ...
        if args.seed:
            assert re.search(r'Opened FPipelineCacheFile:.*BiellaGames_VULKAN_SM6.upipelinecache', log), 'Seed not opened natively'
        report['result'] = 'PASS'
