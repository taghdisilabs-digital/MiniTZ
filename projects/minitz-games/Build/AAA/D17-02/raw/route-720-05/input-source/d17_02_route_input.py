...
class RouteInput:
    ...
    def tick(self):
        ...
        if s['kind'] == 'rival':
            actors = [a for a in self.actors.values() if a['id'] == s['target']]
            if actors:
                a = actors[0]
                ...
                if not done:
                    ...
                    self.key('w', math.hypot(x, y) > 180 and
                             abs(angle(math.degrees(math.atan2(y,x))-float(self.frame['yaw']))) < 20)
                    self.shoot(aligned and math.hypot(x,y) < 650, now)
        elif s['kind'] == 'walk':
            ...
            if s.get('defend_against_nearest_rival'):
                ...
                self.shoot(bool(nearest and nearest['class'] == 'BiellaRival' and
                                distance_to(nearest) < 1500), now)
        elif s['kind'] == 'look':
            ...
            done = now-self.stage_start > s.get('seconds', 2)
