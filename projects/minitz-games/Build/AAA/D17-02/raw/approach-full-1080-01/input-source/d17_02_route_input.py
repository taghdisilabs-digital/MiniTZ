...
class RouteInput:
    ...
    def shoot(self, aligned, now, preserve_route=True):
        ...
        if preserve_route and self.remaining_infected is not None and self.remaining_infected <= 2:
            ...
            # Actor samples arrive at 10 Hz, and another live rival can defeat
            # an infected before the next objective update. Preserve the last
            # two here, with a margin at nearest-target crossovers.
            infected_distance = min((math.sqrt(distance(a)) for a in live
                                     if 'Infected' in a['class']), default=math.inf)
            if nearest and ('Infected' in nearest['class'] or
                            infected_distance < math.sqrt(distance(nearest)) + 200):
                aligned = False
