...
def main():
    ...
    if plan.get('controller') == 'd17_02_telemetry_guided_input':
        command.append('-BiellaTraversalTelemetry')
        if plan.get('d17_02_vsm_page_diagnostics') is True:
            # Read-only UE renderer counters identify large non-Nanite casters.
            # Opt-in diagnostic captures do not establish performance quality.
            index = command.index('-ExecCmds=t.MaxFPS 60,r.VSync 0')
            command[index] += ',r.Shadow.Virtual.NonNanite.NumPageAreaDiagSlots 32'
    ...
    try:
        ...
        with (output / 'capture.log').open('x') as log_stream:
            ...
            route = None
            if plan.get('controller') == 'd17_02_telemetry_guided_input':
                from d17_02_route_input import RouteInput
                route = RouteInput(plan, output / 'telemetry.jsonl', event)
            while game.poll() is None and time.monotonic()-start < args.seconds:
                ...
                if route:
                    if route.tick():
                        result['reason'] = 'route_inputs_finished'
                        result['route_inputs_finished'] = True
                        break
                    time.sleep(.08)
                    continue
            ...
            result.update(status='CAPTURED', measured_input_run_seconds=time.monotonic()-start,
                          actions_sent=route.actions_sent if route else next_action,
                          game_alive_at_capture_end=game.poll() is None)
