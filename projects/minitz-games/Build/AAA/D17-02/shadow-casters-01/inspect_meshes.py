...
def mesh_state(mesh):
    ...
    return dict(triangles=mesh.get_num_triangles(0),
        bounds_origin=vector(bounds.origin), bounds_extent=vector(bounds.box_extent),
...
result = {}
for asset in ['/Engine/BasicShapes/Cube', '/Game/Environment/ServiceHall/SM_ServiceHallCladding', '/Game/Environment/ServiceHall/SM_HallInterior', '/Game/Environment/TerraceRoute/SM_TerraceRoute']:
    mesh = LIB.load_asset(asset)
    if mesh: result[asset] = mesh_state(mesh)
