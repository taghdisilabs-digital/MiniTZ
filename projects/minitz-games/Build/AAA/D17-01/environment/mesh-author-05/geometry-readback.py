...
ground=bpy.data.collections['SM_ServiceGround editable components']
assert len(ground.objects)==200, len(ground.objects)
ground_records=[]
for obj in ground.objects:
    evaluated=obj.evaluated_get(bpy.context.evaluated_depsgraph_get())
    mesh=evaluated.to_mesh()
    xyz=[evaluated.matrix_world @ v.co for v in mesh.vertices]
    low=[min(v[i] for v in xyz) for i in range(3)]
    high=[max(v[i] for v in xyz) for i in range(3)]
    assert high[2]<=2.01 and low[2]>=-2.01,(obj.name,low,high)
    ground_records.append(dict(name=obj.name,bounds_min=low,bounds_max=high))
    evaluated.to_mesh_clear()
report=dict(ground_parts=ground_records,task_id='D17-01',result='PASS',source_blend_sha256=hashlib.sha256(blend.read_bytes()).hexdigest(),
