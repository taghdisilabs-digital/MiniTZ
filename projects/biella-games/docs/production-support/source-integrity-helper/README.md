...
```bash
python3 projects/biella-games/docs/production-support/source-integrity-helper/check_source_integrity.py --expect-service stopped
```

Run after production is resumed:

```bash
python3 projects/biella-games/docs/production-support/source-integrity-helper/check_source_integrity.py --expect-service active
```
