...
```bash
python3 projects/biella-games/docs/production-support/progress-continuity-helper/check_progress.py snapshot --output /tmp/biella-progress-baseline.json
```

After resume, verify liveness and continuity advancement:

```bash
python3 projects/biella-games/docs/production-support/progress-continuity-helper/check_progress.py check --baseline /tmp/biella-progress-baseline.json --require-forward
```
