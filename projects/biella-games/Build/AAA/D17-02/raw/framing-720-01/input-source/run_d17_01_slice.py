    ...
    try:
        ...
        if args.native_build:
            ...
            if args.task_id == 'D17-02':
                baseline = manifest['source_manifest']
                assert identity(baseline['path']) == baseline, 'Installed source receipt changed'
                previous = json.loads(Path(baseline['path']).read_text())
                assert previous['material_input_digest'] == manifest['source_material_digest']
            else:
                previous = json.loads((PROJECT / 'Build/Release/D08-01/source-build-manifest.json').read_text())
            ...
            permitted = ({'Source/BiellaGames/Private/BiellaPlaytestTelemetry.cpp',
                          ...
                          'Source/BiellaGames/Private/BiellaWorldContinuity.cpp',
                          'Source/BiellaGames/Public/BiellaGamesCharacter.h',
                          'Source/BiellaGames/Private/BiellaGameplayHUD.cpp'} if args.task_id == 'D17-02'
        ...
        command = ['bwrap', '--die-with-parent', '--ro-bind', '/', '/', '--dev-bind', '/dev', '/dev',
                   ...
                   '--ro-bind', inputs['driver']['snapshot']['path'], '/tmp/raw-input.py',
                   '--ro-bind', inputs['plan']['snapshot']['path'], '/tmp/input-plan.json',
                   '--ro-bind', str(output / 'resolved-package.json'), '/tmp/resolved-package.json']
        if args.task_id == 'D17-02':
            command += ['--ro-bind', inputs['route_controller']['snapshot']['path'], '/tmp/d17_02_route_input.py']
        if native:
