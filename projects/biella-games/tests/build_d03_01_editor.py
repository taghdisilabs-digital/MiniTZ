...
def main():
    ...
    report=dict(task_id='D03-01',result='RUNNING',revision=source_revision(),inputs_before=inputs())
    ...
    if report['result']=='PASS':
        ...
        with Path('/mnt/biella-extra/biella-runtime/codex-production/failures.jsonl').open('a') as stream:
            stream.write(json.dumps(dict(task_id='D03-01',time=datetime.now(timezone.utc).isoformat(),type='editor_build',status='CONTINUE',diagnostics=report.get('error','Native build failed'),evidence=str(out)))+'\n')
    write_json(out/'validation.json',report)
