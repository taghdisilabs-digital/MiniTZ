...
def main():
    ...
    parser.add_argument('--native-build', type=Path,
                        help='Passing native HUD-only build directory; original cooked package stays intact')
    ...
    try:
        ...
        if args.native_build:
            ...
            assert changed == ['Source/BiellaGames/Private/BiellaGameplayHUD.cpp'], changed
            native = Path('/root/biella/artifacts/games/D17-01') / source['binary']['sha256'] / 'BiellaGames'
            ...
            resolved.update(native_build=identity(args.native_build / 'result.json'),
                            ...
                            cook_reuse='All content/config/reflected declarations/shaders unchanged; native HUD layout only',
        ...
        with LEDGER.open('a') as stream:
            stream.write(json.dumps({'task_id': 'D17-01', 'time': now(),
