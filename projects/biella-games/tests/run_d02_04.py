#!/usr/bin/env python3
"""Run native environment input/Chaos/streaming lifecycle and preserve raw evidence."""
import argparse
...
def main():
    ...
    try:
        cmd = ['runuser', '-u', account.pw_name, '--', 'env', 'SDL_AUDIODRIVER=dummy', 'xvfb-run', '-a', '-s', '-screen 0 1280x720x24',
               ...
               f'-ExecCmds=t.MaxFPS {args.fps},r.VSync 0,r.ScreenPercentage 100,r.DynamicRes.OperationMode 0,Automation RunTests BiellaGames.D02.Environment; SoftQuit']
        ...
        assert re.search(r'Result=\{Success\}.*Name=\{Environment\}', log), 'Environment automation failed'
