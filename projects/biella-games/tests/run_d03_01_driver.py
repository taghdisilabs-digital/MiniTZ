#!/usr/bin/env python3
"""Validate skeletal seated driver presentation using the existing input/Chaos lifecycle."""
import argparse
...
from verify_d03_01_driver import verify
...
def main():
    ...
    def inputs():
        return identities(DEFAULT_EDITOR) + [file_identity(Path(__file__))] + [
            file_identity(PROJECT / 'tests' / name) for name in ('verify_d03_01_driver.py', 'verify_d03_01_vehicle.py', 'verify_d02_03.py')]

    report = dict(task_id='D03-01', result='FAIL', revision=source_revision(), disabled=args.disabled,
                  scope='seated driver pelvis/footwell/steering contact, authority preservation, fallback, exit and defeat',
    ...
    try:
        command = ['runuser', '-u', 'unreal', '--', 'env', 'SDL_AUDIODRIVER=dummy',
                   ...
                   f'-AbsLog={out}/runtime.engine.log', f'-BiellaVehicleOutput={out}', '-BiellaVehiclePresentation', '-BiellaDriverPresentation',
        ...
        if args.disabled:
            command.append('-BiellaDriverPresentationDisabled')
        ...
        with Path('/mnt/biella-extra/biella-runtime/codex-production/failures.jsonl').open('a') as stream:
            stream.write(json.dumps(dict(task_id='D03-01', time=datetime.now(timezone.utc).isoformat(),
                                         type='driver_presentation_validation', status='CONTINUE',
