...
void ABiellaEnvironmentSite::BeginPlay()
{
    ...
    auto* Base=LoadObject<UMaterialInterface>(nullptr,TEXT("/Game/Materials/M_DemoReadability.M_DemoReadability"));
    auto Material=[this,Base](FLinearColor Color)
    {
        ...
        M->SetScalarParameterValue(TEXT("ReadabilityFill"),0.12f);
        return M;
    };
    ...
    for (int32 I=0;I<3;++I)
    {
        auto* M=Material(FLinearColor(0.24,0.38,0.42)); PanelMaterials.Add(M);
    ...
    {
        Add(Side<0 ? TEXT("PostLeft") : TEXT("PostRight"),FVector(0,Side*255,115),FVector(28,28,230),Frame,true);
        Add(Side<0 ? TEXT("RailLeft") : TEXT("RailRight"),FVector(270,Side*235,35),FVector(520,14,60),Frame,true);
    }
    Add(TEXT("Header"),FVector(0,0,245),FVector(30,550,30),Frame,true);
    Add(TEXT("Cabinet"),FVector(-170,-310,80),FVector(65,65,160),Frame,true);
    ...
    WarningLight->SetupAttachment(RootComponent); WarningLight->SetRelativeLocation(FVector(240,0,220));
    WarningLight->SetLightColor(FLinearColor(1,0.08,0.005)); WarningLight->SetIntensity(4500);
    WarningLight->SetAttenuationRadius(700); WarningLight->SetCastShadows(false); WarningLight->RegisterComponent();
    Sign=NewObject<UTextRenderComponent>(this); AddInstanceComponent(Sign);
    Sign->SetupAttachment(RootComponent); Sign->SetRelativeLocation(FVector(-20,0,295));
    Sign->SetRelativeRotation(FRotator(0,180,0)); Sign->SetHorizontalAlignment(EHTA_Center);
    Sign->SetWorldSize(30); Sign->SetTextRenderColor(FColor::White); Sign->RegisterComponent();
...
void ABiellaEnvironmentSite::UpdatePresentation()
{
    ...
    PowerMaterial->SetScalarParameterValue(TEXT("ReadabilityFill"),bPowered ? 0.9f : 0.12f);
    WarningLight->SetVisibility(bPowered && !bDormant);
    Sign->SetText(FText::FromString(bPowered ? TEXT("DANGER / LIVE FLOOR") : TEXT("SERVICE BAY / POWER OFF")));
}
