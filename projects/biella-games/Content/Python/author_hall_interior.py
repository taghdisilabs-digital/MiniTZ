"""Import fitted hall interior detail without editing original envelopes or colliders.
...
SPEC_PATH=PROJECT/'SourceAssets/Environment/hall-interior.json'
SPEC=json.loads(SPEC_PATH.read_text())
VERIFY='-D17VerifyHallInterior' in unreal.SystemLibrary.get_command_line()
...
def require(ok,message):
    if not ok: raise RuntimeError('D17_HALL_INTERIOR FAIL: '+message)
...
'D02_Interior_Floor':([8000,2250,-50],[20,15,1])}
supports={}
...
if not VERIFY:
    ...
    a.set_editor_property('tags',[unreal.Name('D17HallInteriorVisual')])
...
report=dict(task_id='D17-01',result='PASS',status='GENERATED_DRAFT',read_only=VERIFY,
    ...
    scope='Fitted hall interior detail; unchanged supporting envelopes, collision, lights and gameplay; no slice acceptance')
Path(os.environ['BIELLA_D17_HALL_INTERIOR_REPORT']).write_text(json.dumps(report,indent=2)+'\n')
print('D17_HALL_INTERIOR COMPLETE')
