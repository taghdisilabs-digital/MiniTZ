...
OutColor=lerp(lerp(concrete,steel,inside),paint,wear)*lerp(1.0,.72,damp);
OutMetal=inside*.78*(1.0-wear)*(1.0-.35*age);
OutRough=lerp(lerp(.78,.39,inside)+grit*.025,.095,puddle);
...
float3 gradient=float3(slope,0)*.018*filtered*(1.0-puddle);
