...
BRIDGE = ROOT / "docs/project-state/MINITZ_ISOLATED_PROJECT_EXECUTION_BRIDGE.yaml"
SOURCE_MAP = ROOT / "docs/project-state/06_MINITZ_SOURCE_EVIDENCE_MAP.yaml"
...
DRIVE = ROOT / "docs/project-state/MINITZ_DRIVE_LIVE_MANIFEST.md"
...
def test_approved_bridge_is_registered_as_current_machine_guidance():
    ...
    assert "MINITZ_ISOLATED_PROJECT_EXECUTION_BRIDGE.yaml" in text
...
def test_donor_project_cell_guidance_is_preserved_while_current_minitz_policy_is_native():
    ...
    assert "MiniTZ owns task execution" in instructions
...
def test_drive_manifest_points_to_exact_bridge_object_and_git_counterpart():
    ...
    assert "MINITZ_ISOLATED_PROJECT_EXECUTION_BRIDGE.yaml" in text
    assert DRIVE_ID in text
    assert "docs/project-state/MINITZ_ISOLATED_PROJECT_EXECUTION_BRIDGE.yaml" in text
    assert BRIDGE_SHA in text
