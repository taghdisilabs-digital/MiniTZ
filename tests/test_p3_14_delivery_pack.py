...
from minitz_os.engine.task import TaskRef
...
def _manifest(project_ref: ProjectRef) -> PackageManifest:
    ...
    return PackageManifest.create(project_ref, "pkg-main", task, run, graph, "wheel", "target://python/wheel/v1", (source,), (entry,), "source://git/main/v1", "build://minitz/v1", {"python": "3.14"}, "b" * 64, None, "2026-09-01T00:00:00+00:00")
...
def test_package_manifest_isolated_entries_and_canonical_identity_fail_closed() -> None:
    ...
    with pytest.raises(DeliveryContractError, match="duplicate"):
        PackageManifest.create(project_ref, "pkg-main", manifest.task_ref, manifest.run_ref, manifest.graph_ref, "wheel", "target://python/wheel/v1", manifest.sources, (manifest.entries[0], manifest.entries[0]), "source://git/main/v1", "build://minitz/v1", {}, "b" * 64, None, "2026-09-01T00:00:00+00:00")
...
def test_publish_scope_request_and_receipt_states_are_fail_closed() -> None:
    ...
    request = PublishRequest.create(manifest, destination, "target://python/wheel/v1", "release://minitz/v1", "upload", False, "verification://sha256/v1", manifest.task_ref, manifest.run_ref, node, "idem_delivery_001")
    ...
    with pytest.raises(DeliveryContractError, match="Project"):
        PublishRequest.create(manifest, PublishDestination(ProjectRef.new(), False, "adapter://delivery/http/v1", "registry", "endpoint://registry/example/v1", "secret://publish/token/v1", ("target://python/wheel/v1",), "policy://data/internal/v1"), "target://python/wheel/v1", "release://minitz/v1", "upload", False, "verification://sha256/v1", manifest.task_ref, manifest.run_ref, node, "idem_delivery_002")
