...
TEST_ROOT="$(mktemp -d /tmp/minitz-ai-smoke.XXXXXX)"
...
ln -s "$ROOT_DIR/ops/local-ai/minitz-ai-start.sh" "$TEST_ROOT/bin/minitz-ai-start"
...
OUTPUT="$(PATH="$TEST_ROOT/bin:$PATH" \
  ...
  MINITZ_AI_RUNTIME_ROOT="$RUNTIME_ROOT" \
  MINITZ_AI_STATE_ROOT="$STATE_ROOT" \
  MINITZ_AI_LOG_ROOT="$LOG_ROOT" \
  MINITZ_GATEWAY_URL="http://127.0.0.1:8787" \
  "$TEST_ROOT/bin/minitz-ai-start" 2>&1)"
EXIT_CODE=$?
set -e
...
grep -F --quiet -- 'Codex:        READY (Saturn MCP registered; use minitz-codex)' <<< "$OUTPUT"
