...
def test_failure_learning_public_states_and_project_scope(tmp_path: Path) -> None:
    from minitz_os.engine.failure_repair_learning import FailureLearningService, FailurePatternScope, FailurePatternStatus, RootCauseState, RepairOutcome, FailureMatchState
...
def test_repair_learning_retains_failures_redacts_and_rebuilds_index(tmp_path: Path) -> None:
    from minitz_os.engine.failure_repair_learning import FailureLearningService, RepairOutcome
    service = FailureLearningService(tmp_path / "learning.db")
    project = ProjectRef.new()
...
def test_observation_normalizes_volatile_ids_and_redacts_sensitive_evidence(tmp_path: Path) -> None:
    from minitz_os.engine.failure_repair_learning import FailureLearningService
    service = FailureLearningService(tmp_path / "learning.db")
...
def test_match_states_and_similar_text_different_cause_do_not_merge(tmp_path: Path) -> None:
    from minitz_os.engine.failure_repair_learning import FailureLearningService, FailureMatchState
...
def test_root_cause_contradiction_preserves_versions_and_repair_validation_is_required(tmp_path: Path) -> None:
    from minitz_os.engine.failure_repair_learning import FailureLearningService, RootCauseState, RepairOutcome
