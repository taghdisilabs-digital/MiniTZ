...
MODULE = LOCAL_AI / "minitz_task_packet.py"
spec = importlib.util.spec_from_file_location("minitz_task_packet", MODULE)
...
def test_packet_contains_exact_active_contract_and_resource_rule(tmp_path: Path):
    ...
    (repo / "docs/project-state/04_MINITZ_ACTIVE_TASK.md").write_text(active, encoding="utf-8")
    ...
    production = state.ProductionState(Path("/repo/projects/minitz-games"), "IN_PROGRESS", "demo01", "D01-030", [section])
    packet = packets.compile_task_packet(repo, production, task)
    assert "TASK: D01-030 [hard] Rival combat" in packet
    assert "04_MINITZ_ACTIVE_TASK.md" in packet
...
def test_task_memory_capsule_is_bounded_and_project_aware(tmp_path: Path):
    ...
    capsule = packets.build_task_memory_capsule(
        ...
        dirty_paths=["Source/MiniTZGames/Private/MiniTZWorldContinuity.cpp", "tests/run_d02_01.py"],
    )
    assert capsule["task_id"] == "D02-01"
    assert capsule["project_root"] == str(project)
    assert capsule["session_id"] == "session-d02"
    assert capsule["dirty_paths"] == ["Source/MiniTZGames/Private/MiniTZWorldContinuity.cpp", "tests/run_d02_01.py"]
...
def test_task_packet_autonomously_provides_routine_needs(tmp_path: Path):
    ...
    production = state.ProductionState(Path("/repo/projects/minitz-games"), "IN_PROGRESS", "post_d01", "D02-01", [state.SectionRecord("post_d01", "Continuation", "IN_PROGRESS", [task])])
...
def test_bounded_fallback_packet_is_self_contained_retrieval_first_and_quality_locked(tmp_path: Path):
    task = state.TaskRecord("D03-01", "hard_creation", "Production quality", "PENDING", (), "post_d01")
    memory = tmp_path / "task-memory/D03-01.json"; memory.parent.mkdir(parents=True)
    projection = tmp_path / "memory/current-task.json"; projection.parent.mkdir(parents=True)
    guide = tmp_path / "docs/task-guides/D03-01.md"; guide.parent.mkdir(parents=True)
    memory.write_text('{"summary":"frontier-memory-marker"}\n')
    projection.write_text('{"task_memory":{"summary":"projection-marker"}}\n')
    guide.write_text('# Guide\nportable-seed-guide-marker\n')
    packet = packets.compile_bounded_fallback_packet(task, memory, projection, guide)
    lower = packet.lower()
    ...
    assert str(memory) in packet and str(projection) in packet and str(guide) in packet
    assert "frontier-memory-marker" in packet
    assert "projection-marker" in packet
    assert "portable-seed-guide-marker" in packet
    assert "--- TASK MEMORY CONTENT ---" in packet
    assert "--- TASK GUIDE CONTENT ---" in packet
    assert len(packet.encode()) < 30000
