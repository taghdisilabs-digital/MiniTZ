...
def test_render_frame_and_manifest_fail_closed() -> None:
 p=ProjectRef.new(); c=_config(p); r=_request(p,c); f=RenderFrameRef.from_request(r,"artifact://frame/1/v1","d"*64,True)
 m=RenderSequenceManifest(p,r,(1,),(f,),(),"e"*64); assert m.require_frame(f) is f
 multi=RenderSequenceManifest(p,r,(1,),(f,RenderFrameRef.from_request(r,"artifact://frame/1/depth/v1","f"*64,True,"depth")),(),"e"*64); assert len(multi.completed_frames)==2
 child=replace(RenderFrameRef.from_request(r,"artifact://frame/1/child/v1","9"*64,True,"normal"),node_ref="node://render/child/v1",executor_ref="executor://worker/v1",dependencies=("dispatch://child/v1",)); assert len(RenderSequenceManifest(p,r,(1,),(child,),(),"e"*64).completed_frames)==1
 with pytest.raises(RenderContractError):RenderSequenceManifest(p,r,(1,),(f,RenderFrameRef.from_request(r,"artifact://frame/1/duplicate/v1","0"*64,True)),(),"e"*64)
