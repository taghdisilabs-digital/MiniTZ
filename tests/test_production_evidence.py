...
MODULE = LOCAL_AI / "minitz_production_evidence.py"
spec = importlib.util.spec_from_file_location("minitz_production_evidence", MODULE)
...
def test_complete_requires_evidence(tmp_path: Path):
    ...
    assert result.status == "COMPLETE" and result.evidence == ()
...
def test_service_style_typed_completion_resolves_active_repo_src_without_pythonpath(tmp_path: Path):
    ...
    env = dict(os.environ); env.pop("PYTHONPATH", None); env["MINITZ_REPO_ROOT"] = str(ROOT)
...
def test_continue_does_not_dirty_durable_state(tmp_path: Path):
    repo, project = fixture(tmp_path)
    before03 = (repo / "docs/project-state/03_MINITZ_CURRENT_STATE.md").read_bytes()
    before04 = (repo / "docs/project-state/04_MINITZ_ACTIVE_TASK.md").read_bytes()
    result = evidence.TaskResult("D01-030", "CONTINUE", "more work", ("partial",))
    evidence.apply_result(repo, project, result, routing.Route("gpt-6-astra", "ultra"))
    assert (repo / "docs/project-state/03_MINITZ_CURRENT_STATE.md").read_bytes() == before03
    assert (repo / "docs/project-state/04_MINITZ_ACTIVE_TASK.md").read_bytes() == before04
...
def test_persist_continuity_commits_and_pushes_exact_main(tmp_path: Path):
    ...
    current = repo / "docs/project-state/03_MINITZ_CURRENT_STATE.md"
    current.write_text(current.read_text() + "# continuity update\n")
    identity = evidence.persist_continuity(repo, "D01-030")
...
def _source_pair(root: Path):
    ...
    subprocess.run(["git", "-C", str(other), "config", "user.name", "MiniTZ Test"], check=True)
