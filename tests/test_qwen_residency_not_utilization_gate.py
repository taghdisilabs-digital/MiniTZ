...
LIBRARY = ROOT / "ops/workstation/biella-lib.sh"
...
def check_resident(models):
    ...
    command = 'source "$1"; fixture="$2"; curl() { printf "%s" "$fixture"; }; biella_verify_qwen_vram'
...
def test_resident_model_is_not_rejected_by_old_fixed_vram_ceiling(mib):
    result = check_resident([{"name": "qwen3-coder-next:biella", "size_vram": mib * 1024**2}])
    assert result.returncode == 0, result.stderr
    assert int(result.stdout.strip()) == mib * 1024**2


@pytest.mark.parametrize("models", [[], [{"name": "other", "size_vram": 1024}],
    [{"name": "qwen3-coder-next:biella", "size_vram": 0}],
    [{"name": "qwen3-coder-next:biella", "size_vram": -1}],
    [{"name": "qwen3-coder-next:biella", "size_vram": "UNKNOWN"}],
    [{"name": "qwen3-coder-next:biella", "size_vram": True}]])
